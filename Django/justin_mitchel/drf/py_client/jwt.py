...
@dataclass
class JWTClient:
  # Use a dataclass decorator to siimplify the class construction
  access:str = None
  refresh:str = None
  ...
  base_endpoint = 'http://localhost:8000/api_06'
  ...
  def perform_auth(self):
    """
    Simple way to perform authentication witout exposing password(s)
  ...
  def write_creds(self, data:dict):
    """
    Store credentials as a local file and update instance with correct
    data.
    """
  ...
  def verify_token(self):
    """
    Simple method for verifying your token data. This method only 
    verifies your `access` token. A 200 HTTP status means success, 
  ...
  def clear_tokens(self):
    """
    Remove any/all JWT token data from instance as well as stored
    creds file.
    """
    self.access = None
    self.refresh = None
  ...
  def perform_refresh(self):
    """
    Refresh the access token by using the correct auth headers and 
    the refresh token.
    """
  ...
  def list(self, endpoint=None, limit=3):
    """
    Here is an actual api call to a DRF View that requires our 
    simplejwt Authentication working correctly.
    """
    headers = self.get_headers()
    if endpoint is None or self.base_endpoint not in str(endpoint):
      endpoint = f'{self.base_endpoint}/products_20/?limit={limit}'
...
if __name__ == '__main__':
  """
  Here's Simple emxample of how to use our client above.

  this will either prompt a login process or just run with current
  stored data
  """
  client = JWTClient()

  """
  simple instance method to perform an HTTP request to our 
  /api/products_20/ endpoint
  """
  ...
  if next_url:
    look_2_data = client.list(endpoint=next_url)
    results += look_2_data.get('results')
