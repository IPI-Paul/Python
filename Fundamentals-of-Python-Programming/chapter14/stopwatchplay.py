# Listing 14.5
# Provides some sample client code that uses the CountingStopwatch class in a 
...
def draw_watch(sw):
    ...
    # Copute time in hours, minutes, seconds
    ...
    # each digitoccupies two spaces; pad with leading zeros, if necessary
    time = '{0:0>2}:{1:0>2}:{2:0>2}'.format(hours, minutes, seconds)
    
    # Draw gvraphical string
    t.clear()
    t.penup()
    t.setposition(-200, 0)
    t.pendown()
...
def update():
    ...
    t.ontimer(update, 500)  # Call the update function again after half a second
...
t.ontimer(update, 500)      # Call update function every half a second
