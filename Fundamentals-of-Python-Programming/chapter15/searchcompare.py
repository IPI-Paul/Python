# Listing 15.7
# Gives an empirical algoritm
# Author: Rick Halterman
# Last modified: 

"""
Compares the running times of linear search and binary search on lists of 
various sizes
"""

def binary_search(lst, seek):
    """
    Returns the index of element seek in list lst, if seek is present in the 
    list.
    lst must be in sorted order. Returns None if seek is not an element of lst
...
def ordered_linear_search(lst, seek):
    ...
    lst: is the list in which to serach
    seek: is the element to find
    """
    i = 0
    n = len(lst)
...
def run_search(lst, seeks, search, trials):
    """
    Searches for all the elements in an ordered list (lst) using search function
    search. Averages the running time over trials run. Return the average time
    """
...
def test_searches(lst, seeks, trials):
    """
    Measures running times of ordered linear search and binary search on a given 
    list. Averages the times over n runs
    """
    # Find each element using ordered linear search
    lin = run_search(lst, seeks, ordered_linear_search, trials)
    # Find each element using ordered binary search
    bin = run_search(lst, seeks, binary_search, trials)
    # Print the results
    print('{0:6} {1:10.5f} {2:10.5f} {3:8.1f}'.format(len(lst), lin, bin, 
                                                      lin / bin))
...
def main():
    """
    Makes a table comparing the running times of ordered linear search vs binary
    search on lists of various sizes
    """
