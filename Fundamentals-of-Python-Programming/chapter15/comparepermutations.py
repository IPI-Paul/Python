# Listing 15.19
# Runs each permutation function 1,000,000 times on the list [1, 2, 3] and 
# tallies each permutation. There are exactly six possible permutations of this
# theree-element list
...
def report(a):
    """
    Reports the accumulated statistics
    """
    print('1, 2, 3: ', a[0])
    print('1, 3, 2: ', a[1])
    print('2, 1, 3: ', a[2])
...
def run_test(perm, runs):
    """
    Uses a permutation function to generate the permutations of the list 
    [1, 2, 3]
    perm: the permutation function to test
    runs: the number of permutations to perform
    """
    # The list to permute
    original = [1, 2, 3]
    ...
    for i in range(runs):   # Run runs times
        # working holds a copy of original is gets permuted ans tallied
        working = original[:]
        # Permute the list with the permutaion algorithm
        perm(working)
