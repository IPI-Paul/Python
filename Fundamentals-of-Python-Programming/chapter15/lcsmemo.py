# Listing 15.24
# Utilises both the memoized and non-memoized versions of LCS functions (longest
# common subsequence). It performs multiple runs on the same data to achieve 
# more accurate measurements. It then plots the results using Turtle graphics
...
def LCS(X, Y):
    ...
    if len(X) > 0 and len(Y) > 0:# No symbols in common if either string is empty
        ...
        if X[0] == Y[0]: # Do the first symbols of both strings match?
            # An LCS will include this shared symbol plus the LCS of rest of both
            # strings
...
def LCS_memoized(X, Y):
    """
    Computes the longest common subsequence of substrings X and Y. Caches 
    intermediate results in a memoization dictionary so they do not need to be 
    ...
    def LCS(X, Y):
        ...
        if(X, Y) in memo:
            # LCS of X and Y already computed, so use previous value
            result = memo[X, Y] 
        else:
            result = '' # No symbols in common unless determined otherwise
            # No symbols in common if either string is empty
            if len(X) > 0 and len(Y) > 0:
                ...
                if X[0] == Y[0]: # First symbols of both strings match
                    ...
                    # Choose longest
                    result = X_Y1 if len(X_Y1) > len(X1_Y) else X1_Y
            # Remember this result for X and Y to avoid resomputing it in future
            memo[X, Y] = result
        return result
...
def compare_LCS(seq1, seq2):
    """
    Computes the longest common subsequence of seq1 and seq2 in two different 
    ways and compares the execution times of the two approaches
    """
    ...
    print(seq1)
    print(seq2)
    print(subseq_std, len(subseq_std), '(Standard: {:f})'.format(
        timer_std.elapsed()))
    print(subseq_memo, len(subseq_memo), '(Standard: {:f})'.format(
        timer_memo.elapsed()))    
    print()
...
def build_random_string(n, symbols):
    """
    Builds a string of length n with characters selected pseudorandomly from the 
    string parameter symbols
    """
    result = ''
    while len(result) < n:
        result = result + choice(symbols) # Add a randonm character from symbols
    return result

def plot(data):
    """
    Plots the data from the LCS vs Memoized LCS exirements
    """
    
    # Computes bounds for the graphing
    w = window_width()
    h = window_height()
    x_origin = -w / 2 + 15
    y_origin = -h / 2 + 15
    ...
    def plot_curve(index, color):
        ...
        color: is the line and point colour
        """
        penup()
        pencolor(color)
...
def main():
    ...
    # Make a list of tuples for measuring the time to compute the LCS two 
    # different ways; initailise all the times to zero
    data = [(0, 0) for _ in range(max_length)]
    
    # Perform the series of experiments N times to produce more reliable 
    # results
    for runs in range(N):
        # The series of experiments consists of computing the LCS of two strings 
