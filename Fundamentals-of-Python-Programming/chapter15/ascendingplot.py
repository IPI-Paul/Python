# Listing 15.1
# Performs an experiment to test a mathematical analysis. It compares the 
# performance of two algorithms on lists of growing lengths
...
def is_ascending(lst):
    """
    Returns True if lst contains elements in nondecreasing order as determined 
    by the < operator. Returns False if the elements of lst are not in order. 
    Throws an exception if lst is not a list or its elements are not compatible 
    with the < operator
    """
...
def compute_time(size, data1, data2):
    """
    Compares the performance of the is_ascending and is_ascending2 functions on 
    a list of a given size. Prints the results of the executions and appends the 
    results to the data1 and data2 lists for further processing
    """
    
    print('List size:', size)
    my_lst = list(range(size))      # Make list [0, 1, 2, 3, ..., size -1]
    start = clock()                 # Start the clock
    ans = is_ascending(my_lst)      # Compute answer
    elapsed1 = clock() - start      # Stop the clock
    print('   is_ascending:   {} Elapsed: {:12.7f}'.format(ans, elapsed1))
    start = clock()                 # Start the clock
    ans = is_ascending2(my_lst)      # Compute answer
