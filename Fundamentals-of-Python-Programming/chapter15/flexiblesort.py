# Listing 15.4
# Arrangesthe elements in a list two different ways using the same selection_sort
# function
...
def random_list():
    """
    Produce a list of pseudorandom integers. The list's length is chosen 
    pseudorandomly in the range 3-20. the integers in the list range from -50 to 
    50
    """
...
def selection_sort(lst, cmp):
    """
    Aranges the elements of list lst in ascending order. The comparer function 
    is used to order the elements. the contents of lst are physically rearranged
    """
    n = len(lst)
    for i in range(n - 1):
        # Note: i, small, and j represent positions within lst
        # lst[i], lst[small], and lst[j] represent the elements at those positions
        # small is the position of the smallest value we've seen so far; we use 
        # it to find the smallest value less than lst[i]
        small = i
