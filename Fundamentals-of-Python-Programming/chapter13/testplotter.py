# Listing 13.15
# Exercises the plotobj module, demonstrating its use within a program
...
def arrow_wheel(plotter, x, y, len, angle, color):
    """
    Draws a collection of arrows extending radially from an (x, y) center point.
    ...
    xe, ye = len, 0.0
    for i in range(360 // angle):   # Integer division
...
def main():
    ...
    def run_test(x, y):
        """
        Generate an arrow wheel centered at (x , y) with random size and color
        """
        # test_arrow(plt)
    ...
    # Plot f(x) = 1/28x + 3, for -10 <= x < 100
    plt.plot_function(quad, 'red')
    # Pot f(x) = x, for -10 <= x < 100
