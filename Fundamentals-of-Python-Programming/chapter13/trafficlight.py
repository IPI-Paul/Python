# Listing 13.8
# Provides the class definition for traffic light objects
...
class TrafficLight:
    ...
    def __init__(self, x, y, width, canvas, initial_color = 'red'):
        ...
        # 1/5 of width will be the basic unit for computing the height of the 
        # traffic light and positioning the individual lamps.
        ...
        self.red_lamp = canvas.create_oval(x, y, x + diameter, y + diameter,
                                           fill = red_fill)
        y += 4 * unit
        # Yellow lamp, currently off
        self.yellow_lamp = canvas.create_oval(x, y, x + diameter, y + diameter,
                                           fill = yellow_fill)
        y += 4 * unit
        # Green lamp, currently off
        self.green_lamp = canvas.create_oval(x, y, x + diameter, y + diameter,
                                           fill = green_fill)
    ...
    def change(self):
        ...
        if self.color == 'red':
            ...
            self.canvas.itemconfigure(self.green_lamp, fill=self.color) # Turn green on
        elif self.color == 'green':
            self.color = 'yellow'
            self.canvas.itemconfigure(self.green_lamp, fill='black')  # Turn green off
            self.canvas.itemconfigure(self.yellow_lamp, fill=self.color) # Turn yellow on    
        elif self.color == 'yellow':
            self.color = 'red'
            self.canvas.itemconfigure(self.yellow_lamp, fill='black') # Turn yellow off
            self.canvas.itemconfigure(self.red_lamp, fill=self.color) # Turn red on            
