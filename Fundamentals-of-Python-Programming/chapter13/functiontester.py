# Listing 13.12
# Defines the structure of a rudimentary object that we can use to test functions
...
class FunctionTester:
    ...
    def check(self, msg, expected, func, *args):
        """
        Clients pass a human-readable message (msg) to uniquely identify the 
        test case, the expected value that a correct test would produce 
        (expected), the function to test (func), and any arguments that the 
        function under test requires (*args).
        Provides immediate printed feedback about the test's success or failure
        and adjusts error count accordingly. Returns True if the function passed 
        the test; otherwise, returns False.
        """
        result = True   # test passed unless wqe determine otherwise
        print('   ', msg, ' ', end = ' ')
