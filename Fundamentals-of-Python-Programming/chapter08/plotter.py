# Listing 8.17
# Combines the concepts of lambda functions, functions as data, and turtle 
# graphics
# Author: Rick Halterman
# Last modified: 

"""
Provides the plot function that draws the graph of a mathematical function on a 
...
def initialize_plotter(width, height, min_x, max_x, min_y, max_y):
    """
    Initialises the plotter with a window with dimensions width x height y, the 
    x-axis ranging from min_x to max_x, and the y-axis ranging from min_y to 
    max_y. Establishes the global beginning and ending x values for the plot and
    the global x_increment value. Draws the x- and y-axes.
    """
...
def plot(f, color):
    ...
    intialize_plotter. Plots (x, f(x)), for all x in the range 
    x_begin <= x < x_end.
