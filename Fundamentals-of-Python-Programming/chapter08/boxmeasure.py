# Listing 8.24
# Defines functions within functions to make them unavailble to outside callers.
# It also includes a function named volume which computes the box's volumne
# Author: Rick Halterman
# Last modified: 

from math import fabs

def surface_area(x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, x5, y5, z5, 
                 x6, y6, z6, x7, y7, z7, x8, y8, z8):
    """
    Computes the surface area of a renctangular box (cuboid) defined by the 3D 
    points (x, y, z) of its eight corners:
              7------8
             /|     /|
            3----- 4 |
            | |    | |
    ...
    def area(length, width):
        """
        Computes the area of a lenght x width rectangle
        """
        return length * width
    
    # Main code for surface_are function
    #Compute are of front face
    length = fabs(x2 - x1)
    height = fabs(y3 - y1)
    ...
    # Compute and return surface area: front/back, left side/right side, and 
    # top/bottom faces
...
# Getr the coordinates of the box's corners from the user
