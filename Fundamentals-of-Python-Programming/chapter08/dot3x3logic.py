# Listing 8.12
# Provides the game engine for a 3 x 3 connect-the-dots game
...
Provides the logic for a simplified connect the dots game. The code in this 
module determines and enforces the rules of the game. Callers are responsible 
for the game's presentation component, that is, the user experience.
...
The game allows two players, x and y, to alternately add horizontal and vertical
lines to connect the dots. The following shows a game in progress:
...
completed a square:
       ...
       @   @---@
           | Y |
       @   @---@              
If a player connects two dots and does not complete a square, control passes to 
the other player. A player must add a line during his/her turn. The game ends 
when all dots have been connected. The player with more squares wins the game, 
and the game is a draw if both players have two squares each.

This game engine manages 12 lines. Each line is distinguished by its name (a 
...
The game engine manages four squares. Each square is distinguished by its name 
...
The string 'X' represents player X, and the string 'Y' represents player Y.
"""

#-------------------------------------------------------------------------------
# Global variables that maintain the state of the current game. These variables 
# are meant to be used only by code within this file; the underscore prefix 
# discourages their access outside of this module.
#-------------------------------------------------------------------------------

# Boolean variables that keep track of whether or not a line exists between two 
# dots; for example, if _nw_n is true, this means the line identified as 
...
_nw_n = _n_ne = _nw_w = _n_c = _ne_e = _w_c = _c_e = _w_sw = _c_s = _e_se = \
_sw_s = _s_se = False

# The player whose turn it is currently (player X or player Y)
# The value of this variable should be either the string 'X' or the string 'Y'
_current_player = 'X'

# Stores the owner of the given squares.
# If _leftbottom_owner equals string 'X', that means player X owns the 
# left-bottom square. Initially none of the squares have an owner, hence their
# owners are each None.
_lefttop_owner = _righttop_owner = _leftbottom_owner = _rightbottom_owner = None

#-------------------------------------------------------------------------------
# Functions used only by functions within this file.
# Note the names begin with an underscore to discourage access outside of this 
# module.
#-------------------------------------------------------------------------------
...
def _update_square(sq):
    ...
    sq must be one of the strings 'LeftTop', 'RightTop', 'LeftBottom',
        'RightBottom'
    The function checks to see 
       1) if the square currently is not owned by a player and 
       2) is all the lines are in place to complete the square.
    If both conditions are met, it marks the sqaure with thecurrent player and 
    returns True. If one of the players already ones the square, or if not all 
    four lines exist to complete the square, the function simply returns False.
    """
    ...
    if sq == 'LeftTop' and _lefttop_owner == None \
       and _nw_n and _nw_w and _n_c and _w_c:
        ...
        print(sq, _righttop_owner,_n_ne, _ne_e, _c_e, _n_c, _current_player)
        return True
...
def _update_squares():
    """
    Attempts to update the owners of all squares that a new line might affect.
    Returns True if one or more squares receives a new owner; otherwise, the 
...
def add_line(line):
    ...
    The paremeter line must be one of 'Northwesr_North', 'North_Northeast', etc.
    that is, a string representing a line on the game board.
    If the lineis not present, the function adds the line and returns True.
    If the line already is present, the function does not change the state of 
    the board and returns False.
    """
    
    # The function can change one of the following global variables maintaining
    # the state of the game.
    global _nw_n, _n_ne, _nw_w, _n_c, _ne_e, _w_c, _c_e, _w_sw, _c_s, _e_se, \
           _sw_s, _s_se, _current_player
    
    line_added = False      # Unsucessful by default
    if line == 'Northwest_North' and not _nw_n:
        ...
        _s_se = True
        line_added = True
    # If the line was added successfully, check to see if it completes a square
    if line_added and not _update_squares():
        # Turn moves to other player upon successful move
...
def square_owner(sq):
    ...
    sq must be one of the strings 'LeftTop', 'RightTop', 'LeftBottom', or 
       'RightBottom'.
...
def check_line(line):
    ...
    The paremeter line must be one of 'Northwest_North', 'North_Northeast', etc.
    that is, a string representing a line on the game board.
    The function returns False if the line does not yet exist.
    """
...
def winner():
    """
    Returns the winner, 'X' or 'Y', or 'Draw' if the game board is full and X 
    and Y both own two squares. 
    Returns None if open squares still exist, and so the game can continue.
    """
...
def initialize_board():
    ...
    _lefttop_owner = _righttop_owner = _leftbottom_owner = _rightbottom_owner \
        = None
