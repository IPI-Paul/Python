# Listing 8.13
# Provides a graphical presentation using Python's turtle module.
...
def square_to_point(sq):
    ...
    Used to properly place the square's owner when it becomes captured.
    """
    if sq == "LeftTop":
        return (200, 400)
    elif sq == "RightTop":
...
def dot_to_point(dot):
    """
    Maps a dot to its location on the board. Used to render a dot in its proper 
    place.
    """
    if dot == "Northwest":
        return 100, 500
...
def check_squares():
    ...
    """
    # Check ownership of each potential square
...
def mouse_click(x, y):
    """
    Responds to the user click the mouse when the cursor is over the window. 
    Determines if the user clicked on a dot. Activates an initial dot if the 
    initial dot is not already active. Establishes a line to a terminal dot if 
    possible. If the move is illegal, the function produces a messagebox 
    alerting the player.
    """
    global initial_dot
        ...
        if not initial_dot:
            ...
            if add_line(line_name(initial_dot, dot)):
                ...
                title("3x3 Connect the Dots    Current Player: " 
                      + current_player())
                # Check to see if the move captured a square and/or ended the 
                # game
                check_squares()
...
def reset_game():
    """ Reinitialise the game's state for the start of a new game. """
    clearscreen()
    initialize()
    
def line_name(dot1, dot2):
    """
    Returns the name of the line that connects the Dot objects dot1 and dot2. 
    Derives the line's name from the names of the two dot objects. Dsiplays an 
    error messageboxe if the two dots do not participate in a valid line.
    """
    if (dot1 =="Northwest" and dot2 == "North") or \
       ...
       (dot1 == "Southeast" and dot2 == "South"):
        ...
        messagebox.showerror("Invalid Link", "You may not connect those two " +
                             "dots")
        return None

def initialize():
    """ Initialises the graphical presentation and game engine. """
    # Global dot objects
    
    # Intialise game engine
    initialize_board()
    
    # Specify the dmiensions of the window
    screensize(600, 600)
    ...
    # Draw all the dots
    for dot in ("Northwest", "North", "Northeast", "West", "Center", "East",
