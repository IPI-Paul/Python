# Listing 8.14
# Uses the exact same engine as Listing 8.13 but provides a text-output, 
# keyboard-only-input interface to the players
...
def show_player(p):
    """ Determines the string to print in the context of a square's owner. 
    p is a player. """
    if p:
        return ' ' + p
    else:
...
def make_line(move):
    ...
    The move paremeter is one of '01', '12', '03', etc. representing a line
    connecting the dots in the pattern
         ...
         |   |   |
         6---7---8
    The function returns the string that represents the line position between 
    two dots, one of 'Northwest_North', 'North_Ceter', etc. If the dot1, dot2 
    pair represents an invalid combination, for example, (0, 4), the function
