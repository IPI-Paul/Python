# Listing 9.14
# Provides a fully functioning graphical interactive program that models a 
# traffic light
...
def do_button_press():
    ...
    global variables that this function expects to exist.
    ...
    if color == 'red':
        ...
        canvas.itemconfigure(green_lamp, fill=color)    # Turn green on
    elif color == 'green':
        color = 'yellow'
        canvas.itemconfigure(green_lamp, fill='black')  # Turn green off
        canvas.itemconfigure(yellow_lamp, fill=color)   # Turn yellow on    
    elif color == 'yellow':
        color = 'red'
        canvas.itemconfigure(yellow_lamp, fill='black') # Turn yellow off
        canvas.itemconfigure(red_lamp, fill=color)      # Turn red on            
...
# Create a graphical button and ensure it calls the do_button_press function 
# when the user presses it
button = Button(frame, text = 'Change', command = do_button_press)

# Position button in the containing frames first row, first column, and 
# position canvas in the first row, second column (zero origin, like list 
# indices).
