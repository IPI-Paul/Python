# Listing 12.17
# Exercises count_elements with the is_prime and non_neg functions.
...
def non_neg(n):
    """
    Determines if n is nonegative. Raises a TypeError if n is not an integer.
    """
    return n > 0

def count_elements(lst, predicate):
    """
    Counts the number of integers in list lst that are acceptable to a given 
    predicate (Boolean function). 
    Prints an error message and raises a type error if the list contains an 
    element incompatible with the predicate.
    """
    count = 0
    for x in lst:
        try:
            ...
            print(x, 'is not an acceptable element')
