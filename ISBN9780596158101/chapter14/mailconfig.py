...
# Example 14-9
# PyMailGUI's mailconfig user settings module is listed. This program has its own
# version of this module because many of its settings are unique for PYMailGUI
# Author: Mark Lutz
# Last modified: 
...
module: change me to reflect your machine names, sig, and preferences.
This module also specifies some widget style preferences applied to the GUI, as
well as message Unicode encoding policy and more in version 3.0. See also: local 
textConfig.py, for customizing PyEdit pop-ups made by PyMailGUI.
...
popservername = 'localhost'     # 'pop.secureserver.net' see altconfigs/ for others
...
# notes: your ISP may require that you be directly connected to their system:
...
myaddress = 'paul@ipi-international.co.uk'  # 'PP4E@learning-python.com'
...
# set user to None or '' if no login/authentication is required, and set pswd
# to name of a file holding your SMTP password, or an empty string to force 
# programs to ask (in a console, or GUI)
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# (optional) PyMailGUI: name of local one-line text file with your POP password;
# if empty or file cannot be read, pswd is requested when first connecting; pswd 
# not encrypted: leave this empty on shared machines; 
...
sentmailfile = os.path.join(mysourcedir, '../sourceFiles/mail settings/sentmail.txt')
...
# windows, message content view windows, and view window attchment nbuttons;
...
#-------------------------------------------------------------------------------
# (optiona) column at which mail's origianl text should be wrapped for view,
...
# if not okeyToOpenParts, quick-access part buttons will not appear in the GUI,
# and Split saves parts in a directory  but does not open them;  
# verifyPartOpens used by both Split action and quick-access buttons: all 
# known-type parts open automatically on Split if this set to False; 
...
#-------------------------------------------------------------------------------
# (optional) the maximum number of quick-access mail part buttons to show in the 
# middle of view windows; after this many, a'...' button will be displayed, 
# which runs the 'Split' action to extract additional parts; 
#-------------------------------------------------------------------------------
...
# see the book's Chapter 13 for details: this is a limited and temporary 
# approach to Unicode encodings until a new bytes-friendly email package parser
# is provided which can handle Unicode encodings more accurately on message-level
# basis;
...
# non-ASCII header, as well as non-ASCII names in email addresses in headers,
# in composed messages when they are sent; None means use the UTF-8 default,
# which should work for most use cases; None means that fail to decode are 
...
# most N of the most recently arrived mail; older mail outside this set are 
...
# have a lot of amil in your inbox, and your internet or mail server speed makes
# full loads too slow to be practical; PyMailGUI also loads only newly-arrived
# headers, but this seetting is independent of that feature;
#-------------------------------------------------------------------------------
