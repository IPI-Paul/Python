...
# Example 14-3
# The list window classes here inherit from mailtools mail parser class, but the 
# server list window class embeds an instance of the message cache object, which 
# in turn inherits from the mailtools mail fetcher
# Author: Mark Lutz, Paul I Ighofose
# Last modified: 2019-03-24

"""
################################################################################
Implementation of mail-server and save-file message list main windows: one class 
per kind. Code is factored here for reuse: server and file list windows are 
customized versions of the PyMailCommon list window class; the server window maps
actions to a local file.

List windows create View, Write, Reply, and Forward winddows on user actions. 
The server list window is the main window opened on program startup by the 
top-level file; file list windows are opened on demand via server and file list 
window "Open". Msgnums may be temporarily out of sync with server if POP inbox 
changes (triggers full reload here).

Changes here in 2.1:
-now checks on deletes and loads to see if msg in sync with server
...
-also threads save-mail deletes so file write doesn't pause GUI

TBD:
-save-mail file saves still not threaded: may pause GUI briefly, but uncommon - 
unlikeload and delete, save/send only appends the local file
-implementation of local save-mail files as text files with separators is mostly
a prototype: it loads all full mail into memory, and so limits the practical 
size of these files; better alternative: use 2 DBM keyed access files for hdrs
and fulltext, plus a list to map keys to position; in this scheme save-mail files
become directories, no longer readable.
################################################################################
"""

from SharedNames import *           # program-wide globals objects
from ViewWindows import ViewWindow, WriteWindow, ReplyWindow, ForwardWindow
import sys
sys.path.append('..')
import chapter13 as mailtools
...
class PyMailCommon(mailtools.MailParser):
    ...
    # allwindows use same dialogs: remember last dirs
    ...
    def makeWidgets(self):
        ...
        # crosslint listbox and scrollbars
    ...
    def contViewRaw(self, msgnums, pyedit=True):    # do we need full TextEditor?
        for msgnum in msgnums:                      # could be a nested def
            ...
            if not pyedit:
                ...
                browser.pack(exapnd=YES, fill=BOTH)
            else:
                # 3.0/4: more useful PyEdit text editor
    ...
    def onViewFormatMail(self):
        """
        possibly threaded: view selected message - pop up formatted displau not 
        threaded if in savefile list, or messages are already loaded 
    ...
    def contViewFmt(self, msgnums):
        """
        finish View: extract main text, popup view window(s) to display; extracts
        plain text from html text if required, wraps text lines; 
        html mails: show extracted text, then save in temp file and open in web 
        browser; part can also be opened manually from view window, Split or 
        part button; verify html open per mailconfig;
        
        3.0: for html-only mail, main text is str here, but save its raw bytes in
        binary mode to finesse encodings; worth the effort because alot of mail  
        is just html today; this first tried N encoding guesses (utf-8, latin-1, 
        platform dflt), but now gets and saves raw bytes to minimize any 
        fidelity loss; if a part is later opened on demand, it is saved in a 
        binary file as raw bytes in the same way;
        ...
        already has its own encoding headers (these take the form of <meta> html
        tags within <head> sections if present; none are inserted in the html 
        here, as some well-formed html parts have them); IE seems to handle most
        html part files anyhow; always encoding html parts to utf-8 may suffice 
        too: this encoding can handle most types of text;
        """
        for msgnum in msgnums:
            ...
            if type == 'text/html':
                if ((not mailconfig.verifyHTMLTextOpen) or 
                    ...
                    try:
                        from tempfile import gettempdir # or TK HTML viewer?
    ...
    def onWriteMail(self):
        """
        compose a new email from scratch, without fetching others; nothing to 
        quote here, but adds sig, and prefills Bcc with the sender's address if 
        this optional header enabled in mailconfig; From may be in i18N encoded
        in mailconfig: view window will decode;
        """
        starttext = '\n'                    # use auto signature text
        if mailconfig.mysignature:
            starttext += '%s\n' % mailconfig.mysignature
        From = mailconfig.myaddress
        Bcc = (mailconfig.Bcc and From) or '?'
        WriteWindow(starttext = starttext,
                    headermap = dict(From=From, Bcc=Bcc))  # 3.0: prefill bcc
    ...
    def contReply(self, msgnums):
        """
        finish Reply: drop attachments, quote with '>', add signature; presets 
        initial to/from values from mail or config module; don't use original To
        for From: may be many or a listname;  To keep name+<addr> format even if
        ',' separator in name; Uses original From for To, ignores reply-to header 
        is any;
        3.0: replies also copy to all original receipients by default;
        
        3.0: now uses getaddresses/parseaddr full parsing to separate addrs on 
        commas, and handle any commas that appear nested in email name parts; 
        multiple addresses are separated by comma in GUI, we copy comma separators
        when displaying headers, and we use getaddresses to split addrs as needed; 
        ',' is required by servers for separator; no longer uses parseaddr to get
        1st name/addr pair of getaddresses result: use full From for To;
        
        3.0: we decode the Subject header here because we need its text, but the 
        view window superclass of edit windows performs decoding on all displayed
        headers (the extra Subject decode is a no-op); on sends, all non-ASCII 
        hdrs and hdr email names are in decoded form in the GUI, but are encoded 
        within the mailtolls package; quoteOrigText also decodes the initial
        headers it inserts into the quoted text block, and index lists decode 
        for display;
        """
        for msgnum in msgnums:
            ...
            Cc = self.replyCopy(message)                    # 3.0: cc all recipients
            Subj = message.get('Subject', '(no subject)')
            Subj = self.decodeHeader(Subj)                  # decode for str
            if Subj[:4].lower() != 're: ':                  # 3.0: unify case
                Subj = 'Re: ' + Subj
            ReplyWindow(starttext = maintext,
                        headermap = 
                        dict(From=From, To=To, Cc=Cc, Subject=Subj, Bcc=Bcc))
    ...
    def contFwd(self, msgnums):
        """
        finish Forward: drop attachments, quot with '>', add signature; see notes 
        about headers decoding in the Reply action methods; view window superclass 
        will decode the From header we pass here;
        """
        for msgnum in msgnums:
            fulltext = self.getMessage(msgnum)
            message = self.parseMessage(fulltext)
    ...
    def onSaveMailFile(self):
        ...
        if not msgnums:
            ...
            if filename:
                filename = os.path.abspath(filename)    # normalizr / to \
    ...
    def contSave(self, msgnums, filename):
        ...
        if (filename in openSaveFiles.keys() and        # viewing this file?
            openSaveFiles[filename].openFileBusy):      # load/del occuring?
    ...
    def onOpenMailFile(self, filename=None):
        ...
        if filename:
            ...
            if filename in openSaveFiles.keys():        # only 1 win per file
                ...
                openVw = open(filename, 'r', 
                        encoding=mailconfig.fetchEncoding
                        ).read()
                if openVw.split(saveMailSeparator)[1:]:
                    ...
                    message = self.parseMessage(openVw)
                    ctype, maintext = self.findMainText(message)
                    hdrmap = {'From': message['From'], 'To': message['To'], 
                              'Cc': message['Cc'] or '?', 
                              'Subject': message['Subject'] or '?'}
                    ViewWindow(hdrmap, maintext, message)
    ...
    def replyCopy(self, message):
        """
        3.0: replies copy all original recipients, by prefilling Cc header with 
        all addresses in original To and Cc after removing duplicates and new 
        sender; could decode i18n addrs here, but the view window will decode to 
        display (and send will reencode) and the unique set filtering here will 
        work either way, though a sender's i18n address is assumed to be in 
        encoded form in mailconfig (else it is not removed here); empty To or Cc 
        headers are okay: split returns empty lists;
        """
        if not mailconfig.repliesCopyToAll:
            # reply to sender only
            Cc = ''
        else:
            # copy all originalrecipients (3.0)
    ...
    def formatQuotedMainText(self, message):
        """
        3.0: factor out common code shared by Reply and Forward: fetch decoded
        text, extract text if html, line wrap, add > quote
        """
        type, maintext = self.findMainText(message)     # 3.0: decoded str
        if type in ['text/htmls', 'text/xml']:          # 3.0: get plain text
    ...
    def quoteOrigText(self, maintext, message):
        """
        3.0: we need to decode any i18n (internationalised) headers here too, or
        they show up in email+MIME encoded form in the quoted text block; 
        decodeAddrHeader works on one addr or all in a comma-separated list;
        this may trigger full text encoding on sends, but the main text is also
        already in fully decoded form: could be in any Unicode scheme;
        """
        quoted = '\n-----Original Message-----\n'
...
class PyMailFile(PyMailCommon):
    """
    customize PyMailCommon for viewing saved-mail file offline; mixed with a Tk,
    Toplevel, or Frame, adds main mail listbox; maps load, fetch, delete actions 
    to local text file storage; file opens and deletes here run in threads for 
    large files;
    
    save and send not threaded, because only append to file; save is disabled if 
    source or target file busy with load/delete; save disables load, delete, save 
    just because it is not run in a thread (blocks GUI);
    
    TBD: may need thread and O/S file locks if saves ever do run in threads: 
    saves could disable other threads with openFileBusy, but file may not be open 
    in GUI; file locks not sufficient, because GUI updated too;
    TBD: appends to sent-mail file may require O/S locks: as is, user gets error
    pop up if sent during load/del;
    
    3.0: mail save files are now Unicode text, encoded per an encoding name 
    setting in the mailconfig module; this may not support worst case scenarios
    of unusual or mixed encodings, but most full mail text is ascii, and the 
    Python 3.1 email package is partly broken;
    """
    def actions(self):
    ...
    def loadMailFileThread(self):
        """
        load or reload file and update window index list; called on Open, 
        startup, and possibly on Send if sent-mail file appended is currently
        open; there is always a bogus first item after the text split;
        alt: [self.parseHeaders(m) for m in self.msglist];
        could pop up a busy dialog, but quick for small files;
        
        2.1: this is now threaded--else rus < 1sec for N meg files, but can pause 
        GUI N seconds if very ;arge file; 
        Save now uses addSavedMails to append msg lists for speed, not this reload;
        still called from Send just because msg text unavailable - requires refactoring;
        ...
        if self.openFileBusy:
            ...
            # self.listBox.insert(END, 'loading...')    # error if user click
    ...
    def addSavedMails(self, fulltextlist):
        """
        optimization: extend loaded file lists for mail newly saved to this 
        window's file; in past called loadMailThread to reload entire file on 
        save - slow; must be called in main GUI thread only: updates GUI;
    ...
    def doDelete(self, msgnums):
        """
        simple-minded, but sufficient: rewrite all nondeleted mail to file; can't
        just delete from self.msglist in-place: chnages item indexes;
    ...
    def onDeleteMailFileExit(self, savetitle):
        ...
        self.fillIndex()                        # updates GUI: do in main thread
        self.lift()                             # reset my titlte, raise my window
        self.openFileBusy.decr()
    ...
    def getMessages(self, msgnums, after):
        """
        used by view, save, reply, fwd: file load and delete threads may change 
        the msg and hdr lists, so disable all other operations that depend on 
        them to be safe; this test is for self: saves also test target file;
        """
        if self.openFileBusy:
...
class PyMailServer(PyMailCommon):
    """
    customize PyMailCommon for viewing mail still on server; mixed with a Tk,
    Toplevel, or Frame, adds main mail listbox; maps load, fetch, delete actions
    to email server inbox; embeds a MessageCache, which is a mailtools MailFetcher;
    """
    def actions(self):
    ...
    def onShowHelp(self):
        ...
        if mailconfig.showHelpAsText:
            ...
            poputil.HelpPopup(appname, helptext, showsource=self.onShowMySource)
        
        if mailconfig.showHelpAsHTML or (not mailconfig.showHelpAsText):
            from PyMailGuiHelp import showHtmlHelp
            showHtmlHelp()      # 3.0: HTML version without source sile links
    ...
    def onShowMySource(self, showAsMail=False):
        ...
        from chapter13 import(                      # mailtools now a pkg
        ...
        mymods = (
            ...
            chapter13, mailFetcher, mailSender, mailParser,
        ...
        for mod in mymods:
            ...
            if showAsMail:
                ...
                # 4E: assume in UTF8 Unicode encoding (else PyEdit may ask!)
    ...
    def onLoadServer(self, forceReload=False):
        ...
        2.1: cache.loadHeaders may do quick check to see if msgnums in synch 
        with server, if we are loading just newly arrived hdrs;
        """
        if loadingHdrsBusy or deletingBusy or loadingMsgsBusy:
            ...
            self.cache.setPopPassword(appname)   # don't updat GUI in the thread!
    ...
    def onLoadHdrsFail(self, exc_info, popup):
        ...
        if exc_info[0] == mailtools.MessageSynchError:  # synch inbox/index
            ...
            self.cache.password = None                  # force re-input next time
    ...
    def doDelete(self, msgnumlist):
        ...
        2.1: cache.deleteMessages now checks TOP result to see if headers match 
        selected mail, in case msgnums out of synch with mail server: poss if 
        mail deleted by other client, or server deletes inbox automatically - 
        some ISPS may move mail from inbox to undeliverable on load failure;
        """
        if loadingHdrsBusy or deletingBusy or loadingMsgsBusy:
            showerror(appname, 'Cannot delete during load or delete')
        else:
            deletingBusy.incr()
            popup = popuputil.BusyBoxNowait(appname, 'Deleting selected mail')
            threadtools.startThread(
    ...
    def onDeleteExit(self, popup):
        self.fillIndex()                    # no nedd to reload from server
        popup.quit()                        # refill index with updated cahce
    ...
    def onDeleteFail(self, exc_info, popup):
        ...
        self.onLoadServer(forceReload=True) # new threa: some msgnums changed
    ...
    def getMessages(self, msgnums, after):
        ...
        used by save, viewm reply, and forward to prefill cache; may overlap 
        with other loadmasgs and sends, disables delete, load; only runs "after"
        action if the fetch allowed and succeddful;
        2.1 cache.getMessages tests if index in synch with server, but we only 
        test if we have to go to server, not if cached;
        
        3.0: see messagecache note: now avoids potential fetch of mail currently 
        being fetched, if user clicks again while in progress; any message being
        fetched by any other request in progress must disable entire toLoad 
        batch: else, need to wait for N other loads; fetches are still allowed 
        to overlap in time, as long as disjont;
        """
        if loadingHdrsBusy or deletingBusy:
            ...
            if not toLoad:
                ...
                return      # processnow, no wait pop up
            else:
    ...
    def onLoadMsgsFail(self, exc_info, after, popup, toLoad):
        ...
        if exc_info[0] == mailtools.MessageSynchError:  # synch inbox/index
            self.onLoadServer(foreceReload=True)        # new thread: reload
    ...
    def headersMaps(self):
        # list of email.message.Message objects, 3.x requires list() of map()
        # return [self.pareHeaders(h) for h in self.cache.allHdrs()]
