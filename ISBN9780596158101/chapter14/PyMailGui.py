...
# Example 14-1
# Defines the file run to start PyMailGUI. It implements top-level listwindows in 
# the system--combinations of PyMailGUI's application logic and the window protocol
# superclasses
...
See the help string in the PyMailGuiHelp.py for usage details, and a list of
enhancements in this version.

Version 2.0 was a major, complete rewrite. The changes from 2.0 (July '05)
to 2.1 (jan '06) were quick-access part buttons on View windows, threaded loads
and deletes of local save-mail files, and checks for and recovery from message
numbers out-of-synch with mail server inbox on deletes, index loads, and message 
loads.
...
loadFirst changes in Chapter 11); privides an HTML-based version of its help text;
extracts plain-text from HTML main/only parts for display and quoting; supports 
separators in toolbars; addresses both message content and header Unicode/I18N
encodings for fetched, sent, and saved mail (see Ch13 and Ch14); and much more
(see Ch14 for the full rundown on 3.0 upgrades); fetched message decoding happens
deep in the mailtools package, on mail cache load operations here; mailtools also 
fixes a few email package bugs (see Ch13);

This file implements the top-level windows and interface. PyMailGui uses a number
of modules that know nothing about this GUI, but perform related tasks, some of
which are developed in other sections of the book. The mailconfig module is 
expanded for this program.

==Modules defined elsewhere and reused here:==

mailtools (package)
    client-side scripting chapter
    server sends and receives, parsing, contruction      (Example 13-21+)
threadtools.py
    GUI tools chapter
    thread queu management for GUI callbacks             (Example 10-20)
windows.py
    GUI tools chapter
    border configuration for top-level windows           (Example 10-16)
textEditor.py
    GUI programs chapter
    text widget used in mail view windows, some pop ups  (Example 11-14)
...
messagecache.py
    a cache that keeps track of email already loaded
wraplines.py
...
SharedNames.py
    objects shared between classes and main file
ViewWindows.py
...
import mailconfig, sys, _thread as thread, time
...
# directly: only usage; askpassword calls PopupWindow and attaches; the order
# matters here!--PyMail classes redef some method defaults in the Window classes,
# like destroy and okayToExit: must be leftmost; to use PyMailFileWindow
# standalone, imitate logic in PyMailCommon.onOpenMailFile;
################################################################################
...
class PyMailServerWindow(PyMailServer, windows.MainWindow):
    'a Tk, with extra protocol and mixed-in mehtods'
    def __init__(self):
