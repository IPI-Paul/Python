...
# Example 14-4
# Lists the implementation of mail view and edit windows. These windows are 
# created in response to actions in list windows--View, Write, Reply, and Forward 
# buttons. See the callbacks for these actions in the list window module of 
# Example 14-3 for view window initiation calls.
# Author: Mark Lutz, Paul I Ighofose
# Last modified: 2019-03-24

"""
################################################################################
Implementation of View, Wtite, Reply, Forward windows: one class per kind. 
Code is factored here for reuse: a Write window is a customized View window, and 
Reply and Forward are custom Write windows. Windows defined in this file are 
created by the list windows, in response to user actions.

Caveat: 'split' pop ups for opening parts/attachments feel nonintuitive.
2.1: this caveat was address, by adding quick-access attachment buttons.
...
New in 3.0: supports Unicode/mail encodings for headers in mail sent.

TBD: could avoid verifying quits unless text area modified (like PyEdit2.0), but
these windows are larger, and would not catch headers already changed.
...
class ViewWindow(windows.PopupWindow, mailtools.MailParser):
    """
    a Toplevel, with extra protocol and embedded TextEditor; inherits saveParts, 
    partsList from mailtools.MailParser; mixes in custom subclass logic by direct 
    inheritance here;
    """
    # class attributes
    ...
    tempPartDir = '../sourceFiles/TempParts'    # where 1 selected part saved
    ...
    def makeHeaders(self, headermap):
        ...
        packed row frames with fixed-width labels work well too;
        
        3.0: decoding of i18n headers (and email names in address headers) is 
        performed here if still required as they are added to the GUI; some may 
        have been decoded already for reply/forward windows that need to use 
        decoded text, but the extra decode here is harmless for these, and is 
        required for other headers and cases such as fetched mail views; always,
        headers are in decoded form when displayed in the GUI, and will be 
        encoded within mailtools Sends if they are non-ASCII (see Write); i18n 
        header decoding also occurs in list window mail indexes, and for headers
        added to quoted mail text; text payloads in the mail body are also 
        decoded for display and encoded for sends elsewhere in the system (list 
        windows, Write);
        
        3.0: creators of edit windows prefill Bcc header with sender email 
        address to be picked up here, as a convenience for common usages if this 
        header is enabled in mailconfig; Reply also now prefills the Cc header
        with all unique original recipients less From, if enabled;
        """
    ...
    def makePartButtons(self):
        """
        add up to N buttons that open attachments/parts when clicked; alternative 
        to Parts/Split (2.1); okay that temp dir is shared by all open messaged;
        part file not saved till later selected and opened; partname=partname is 
        required in lambda in Py2.4; 
        ...
        def makeButton(parent, text, callback):
            ...
            if mailconfig.partbg: link.cofnig(bg=mailconfig.partbg)
    ...
    def onOnePart(self, partname):
        """
        locate selected part for button and save and open; okay if multiple mail
        open: resaves each time selected; we could probably just use web browser
        directly here; 
        caveat: tempPartDir is relative to cwd - poss anywhere
        caveat: tempPartDir is never cleaned up: might be large, could use 
        tempfile module (just like the HTML main text part display code in 
        onView of the list window class);
        """
        try:
    ...
    def onParts(self):
        """
        show message part/attachments in pop-up window; uses same file naming 
        scheme as save on Split; if non-multipart, single part = full body text
        """
    ...
    def onSplit(self):
        """
        pop up save dir dialog and save all parts/attachments there; if desired,
        pop up HTML and multimedia parts in web browser, text in TextEditor, 
        and well-known doc types on windows; could show parts in View windows 
        where embedded text editor would provide a save button, but most are not
        readable text;
        """
    ...
    def openParts(self, partfiles):
        """
        auto-open well known and safe file types, but only if verified by the 
        user in a pop up; other types must be opened manually from save dir; at 
        this point, the named parts have been already MIME-decoded and saved as 
        raw bytes in binary-mode files, but text parts may be in any Unicode 
        encoding; PyEdit needs to know the encoding to decode, webbrowsers may 
        have to guess or be told;
        caveat: punts for type application/octet-stream even if it has safe 
        filename extension such as .html; 
        caveat: image/audio/video could be opened with the book's playfile.py;
        could also do that if text viewer fails: would start notepad on Windows 
        via startfile; wbbrowser may handle most cases here too, but specific is 
        better;
        """
        ...
        for (contype, fullfilename) in partfiles:
            ...
            # common Windows documents: Word, Excel, Adobe, archives, etc.
                ...
                if self.askOpen(appname, 'Open part "%s"? ' % basename):
                  ...
                  maintype == 'application' and                     # 3.0: +x types
                  extension in ['.eml', '.msg']):    
                fulltext = open(fullfilename, 'r').read()
...
class WriteWindow(ViewWindow, MailSenderClass):
    """
    customize view display for composing new mail inherits sendMessage from 
    mailtools.MailSender
    """
    modelabel = 'Write'
    
    def __init__(self, headermap, starttext, attaches=[]):
        ViewWindow.__init__(self, headermap, starttext)
        MailSenderClass.__init__(self)
        self.attaches = attaches                # each win has own open dialog
    ...
    def onParts(self):
        ...
        if not self.attaches:
            ...
            msg = '\n'.join(['Alread attached:\n'] + self.attaches)
            showinfo(appname, msg)
    
    def onAttach(self):
        """
        attach a file to the mail: name added here will be added as a part on 
        Send, inside the mailtools pkg;
    ...
    def resolveUnicodeEncodings(self):
        ...
        may have had a knowon encoding if this is a reply or forward, but not for
    ...
    def onSend(self):
        ...
        3.0: now fully parses To, Cc, Bcc (in mailtools) instead of splitting
        on the separator naively; could also use multiline input widgets instead
        of simple entry; Bcc added to envelope, not headers;
        
        3.0: Unicode encoding of text parts is resolved here, because it may 
        require GUI promts; mailtools performs the actual emcoding for parts as 
        needed and requested;
        
        3.0: i18n headers are already decoded in the GUI fields here, encoding
        of any non-ASCII i18n headers is performed in mailtools, not here, 
        because no GUI interatcion is required;
        """
        
        # resolve Unicode encoding for text parts;
        bodytextEncoding, attachesEncodings = self.resolveUnicodeEncodings()
        
        # get components from GUI: i18n headers are decoded
    ...
    def onSendExit(self, popup):
        ...
        if sentname in openSaveFiles.keys():                # sent file open?
            window = openSavedFiles[sentname]               # update list, raise
    ...
    def askSmtpPassword(self):
        ...
        caveat: may try this again in thread if no input first time, so goes into
        a loop until input is provided; see pop paswd input logic for nonlooping 
        alternative
        """
        password = ''
        while not password:
...
class ReplyWindow(WriteWindow):
    """
    customize write display for replying text and headers set up by list window
    """
    modelabel = 'Reply'

class ForwardWindow(WriteWindow):
    """
    customize reply display for forwarding text and headers set up by list window
    """
    modelabel = 'Forward'