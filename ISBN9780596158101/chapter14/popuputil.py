...
# Example 14-6
# Implements a handful of utility pop-up windows in a module, in case they ever
# prove useful in other programs
# Author: Mark Lutz
# Last modified: 
...
import sys
sys.path.append('..')
from chapter10.windows import PopupWindow

class HelpPopup(PopupWindow):
    """
    custom Toplevel that shows help text as scrolled text source button runs a 
...
class BusyBoxWait(PopupWindow):
    ...
    main GUI event thread stays alive during wait, but GUI is inoperable during
    this wait state; uses quit redef here because lower, not leftmost;
    """
