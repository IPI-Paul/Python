...
# Example 15-27
# Implements the ability to display any file on a site from the passed-in 
# filename. It assumes the filename is either typed into a web page form or 
# appended to the end of the URL as a parameter
# Author: Mark Lutz
# Last modified: 
...
be passed in a URL param or form field (use "localhost" as the server if local):
    ...
    http://servername/cgi-bin/ch15_getfile.py?filename=somefile.html
    http://servername/cgi-bin/ch15_getfile.py?filename=cgi-bin\somefile.py
    http://servername/cgi-bin/ch15_getfile.py?filename=cgi-bin%2Fsomefile.py

Users can cut-and-paste pr "View Source" to save file locally. On IE, running the 
...
instead. Sending the in the text/HTML mode works on both browsers--text is 
...
if os.path.exists('PyMailCgi/cgi-bin/secret.py'):
...
try:
    ...
    filename = 'cgi-bin\ch15_getfile.py'            # else default filename
