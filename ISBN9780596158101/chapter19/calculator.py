...
# Example 19-20
# Contains the PyCalc source module that puts the ideas to work in the context
# of a GUI. It's a single-file implementation (not counting utilities imported
# and reused). 
# Author: Mark Lutz
# Last modified: 
...
Evaluates expressions as they are entered, catches keyboard keys for expression 
entry; 2.0 added integrated command-line popups, a recent calculations history 
display popup, fonts and colors configuration, help and about popups, preimported
math/random constants, and more;
...
3.0 changes (PP3E):
-use 'readonly' entry state, not 'disabled', else field is greyed out (fix for 
 2.3 tkinter change);
-avoid extended display precision for floats by using str(), instead of 
 'x'/repr() (fix for Python change);
...
-add 'E+' and 'E-' buttons (and 'E' keypress) for float exponents;
 'E' keypress must generally be followed by digits, not + or - optr key;
-remove 'L' button (but still allow 'L' keypress): superflous now, because 
 Python auto converts up if too big ('L' forced this in past);
...
allow '**' as an operator key; allow '+' and 'J' inputs for complex Numbers;
use new decimal type for fixed precision floats; as is, can use 'cmd' popup 
windows to input via main qwindow; 
caveat: PyCalc's precision, accuracy, and some of its behaviour, is currently 
bound by result of str() call;
################################################################################
"""
...
import sys
sys.path.append('..')
from chapter10.guimixin import GuiMixin                     # quit method
from chapter10.widgets import label, entry, button, frame   # widget builders
...
class CalcGui(GuiMixin, Frame):
    ...
    def __init__(self, parent=None, fg=Fg, bg=Bg, font=Font):
        ...
        if not parent or not isinstance(parent, Frame):
            self.master.title('PyCalc 3.0')         # title if owns window
    ...
    def onEval(self):
        self.eval.shiftOpnd(self.text.get())        # last or onlyu opnd
    ...
    def onOperator(self, char):
        ...
        self.erase = 1                              # erased on next opdn|'('
    ...
    def help(self):
        self.infobox('PyCalc', 'PyClac 3.0+\n'
                     'A Python/tkinter calculator\n'
                     'Programming Python 4E\n'
                     'May, 2010\n'
                     '(3.0 2005, 2.0 199, 1.0 1996)\n\n'
