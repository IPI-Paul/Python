...
# Example 19-16
# Shows a variant of parser1 that analyses the string and builds up a parse tree
# that is, a tree of class instances that represents the expression and that may 
    ...
    def Expr(self):
        ...
        while True:
            if self.lex.token in ['\0', ')']:
                ...
                self.lex.scan()
                left = PlusNode(left, self.Factor())    # and root-node
            elif self.lex.token == '-':
                self.lex.scan()
