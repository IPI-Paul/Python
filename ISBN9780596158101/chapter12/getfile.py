...
# Example 12-17
# Implements both the serer-side and the client-side logic needed to ship a 
# requested file from server to client machines over a raw socket
# Author: Mark Lutz
# Last modified: 

"""
################################################################################
implement client and server-side logic to transfer an arbitrary file from server
to client over a socket; uses a simple control-info protocol rather than separate
sockets for control and data (as in ftp), dispatches each client request to a
handler thread, and loops to transfer the entire file by blocks; see ftplib 
examples for a higher-level transport scheme;
################################################################################
"""
...
helptext = """
Usage...
server=> getfile.py -mode server              [-port nnn] [-host hh|localhost]
client=> getfile.py [-mode client] -file fff  [-port nnn] [-host hh|localhost]
"""
