...
# Example 12-4
# Works like the original echo server, but instead forks a new process to handle 
# each new client connection. Because the handleClient function runs in a new 
# process, the dispatcher function can immediately resume its main loop in order 
# to detect and service a new incoming request
# Author: Mark Lutz
# Last modified: 

"""
Server side: open a socket on a port, listen for a message from a client, and 
send an echo reply; forks a process to handle each client connection; child 
processes share parent's socket descriptors; fork is less portable than threads-
-not yet on Windows, unless Cygwin or similar installed;
"""
