...
# Example 5-31
# Uses shared memory to serve as both inputs and outputs of spawned processes. To
# make this work portably, we must create objects defined by the package and pass
# them to Process constructors
# Author: Mark Lutz
# Last modified: 

"""
Use multiprocess shared memory objects to communicate. Passed objects are shared, 
but globals are not on Windows. last test here reflects common use case:
distributing work
"""
...
def updater(val, arr):
    ...
    """
    global count
    count += 1                      # global caount not shared
...
if __name__ == '__main__':
    ...
    for i in range(procs):
        ...
        p.start()
        ps.append(p)
    for p in ps: p.join()           # global count=6 in parent only
    
    #show final results here         # scalar=12: +6 parent, +6 in 6 children
    # array[i]=8: +2 parent, +6 in 6 children
