...
# Example 5-25
# Launches a server and 5 clients in threads running in parallel on the same 
# machine, to communicate over a socket-because all threads connect to the same 
# port, the server consumes the data added by each of the clients
# Author: Mark Lutz
# Last modified: 

"""
sockets for cross-talk communication: start threads to communicate over sockets; 
independent programs can too, because sockets are system-wide, much like fifos;
see the GUI and Internet parts to thte book for more realistic socket use cases;
...
caveat: prints in threads may need to by synchronised if their output overlaps;
"""
...
def server():
    ...
    while True:
        conn, add = sock.accept()               # wait for client to connect
        data = conn.recv(1024)                  # read bytes data frm this client
...
def client(name):
    ...
    sock.send(name.encode())                    # send bytes data from listener
