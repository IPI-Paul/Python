...
# Example 5-36
# Collects in a single module many of the techniques we've met in this chapter. 
# It implements an abstract superclass, LaunchMode, which defines what it means 
# to start a Python program named by a shell command line, but it doesn't define
# how. Instead, its subclasses provide a run method that actually starts a Python 
# program according to a given scheme and (optionally) define an announce method 
...
"""
################################################################################
launch Python progrmas with command lines and reusable launcher scheme classes;
auto inserts 'python' and/or path to Python  executable at front of cammand line;
some of this module may assume 'python' is on your system path (see Lanucher.py);
...
def fixWindowsPath(cmdline):
    """
    change all / to \ in script filename path at front of cmdline; used only by 
    classes which run tools that require this on Windows; on other platforms, 
    this does not hurt (e.g., os.system on Unix);
    """
...
class LaunchMode:
    """
    on call to instance, announce label and run command; subclasses format 
    command lines as required in run(); command should begin with name of the 
    Python script file to run, and not with 'python' or its full path;
    """
...
class Popen(LaunchMode):
    """
    run shell command line in new process
...
class Fork(LaunchMode):
    ...
    def run(self, cmdline):
        ...
        if os.fork() == 0:                          # start new child process
            if not ('__import__' in cmdline and 'system' in cmdline):
...
class Spawn(LaunchMode):
    """
    run python in new process independent of caller for Windows or Unix; use 
    P_NOWAIT for dos box; forward slashes are okay here
    """
    def run(self, cmdline):
...
# may need to specialise the choice elsewhere
#
