...
# Example 11-5
# Is PyPhoto's implementation. Though it gets some utility for free by reusing 
# the thumbnail generation function of the viewer_thumbs module that we 
# originally wrote near the end of Chapter8 in Example 8-45
...
Supports multiple image directory thumb windows - the initial img dir is passed 
in as cmd arg, uses "images" default, or is selected via main window button; 
later directories are opened by pressing "D" in image view or thumbnail windows.

Viewer also scrolls popped-up images that are too laarge for the screen;  still 
to do: (1) rearrange thumbnails when window resized, based on current window 
size; (2) [DONE] option to resize images to fit current window size? (3) avoid 
scrolls if image size is less than window max size: use Label if imgwide <= 
scrwide and imghigh <= scrhigh?

New in 1.1: updated to run in Python 3.1 and latest PIL;

New in 1.0: now does a form of (2) above: image is resized to one of the display's 
dimensions if clicked, and zoomed in or out in 10% increments on key presses; 
generalize me; caveat: seems to lose quality, pixels after many resizes (this is 
probably a limitatio of PIL)

The following scaler adapted from PIL's thumbnail code is similar to the screen 
height scaler here, but only shrinks:
x, y = imgwide, imghigh
...
sys.path.append('..')
from chapter08.viewer_thumbs import makeThumbs  # developed earlier in book
from ipi.imageconvert import ImageConvert

# remember last dirs accross all windows 
...
class ScrolledCanvas(Canvas):
    """
    a canvas in a container that automatically makes vertical and horizontal 
    scroll bars for itself
    """
...
class ViewOne(Toplevel):
    """
    open a single image in a pop-up window when created; a class because 
    photoimage obj must be saved, else erased if reclaimed; scroll if too big 
    for display; on mouse clicks, resizes to mindow's height or width: stretches
    or shrinks; on I/O keypress, zooms in/out; both resizing schemes maintain 
    original aspect ration; code is factored to avoid redundancy here as possible;
    """
    ...
    def drawImage(self, imgpil, forcesize=()):
        try:
            ...
            if sys.platform == 'cygwin':
                try:
                    img = ImageConvert(imgpil.filename)
                    imgtk = img.toPNG()
                    img.remove()
                except:
                    imgtk = imgpil
    ...
    def sizeToDisplaySide(self, scaler):
        ...
        else:                                           # grow: bicubic sharper
            filter = Image.BICUBIC
        if sys.platform == 'cygwin':
            img = ImageConvert(self.imgpath)
            imgnew = img.resizeTo((newwide, newhigh), filter)
            img.remove()
        else:
    ...
    def zoom(self, factor):
        ...
        if sys.platform == 'cygwin':
            img = ImageConvert(self.imgpath)
            new = img.resizeTo((int(wide * factor), int(high * factor)), filter)
            img.remove()
        else:
...
def onDirectoryOpen(event):
    """
    open a new image directory in new pop up available in both thumb and img
    windows
    """
...
def viewThumbs(imgdir, kind=Toplevel, numcols=None, height=400, width=500):
    """
    make main or pop-up thumbnail buttons window; uses fixed-size buttons, 
    scrollable canvas; sets scrollable (full) size, and places thumbs at abs
    x, y coordinates in canvas; no longer assumes all thumbs are same size:
    gets max of all (x, y), some may be smaller;
    """
    win = kind()
    helptxt = '(press D to open other_'
