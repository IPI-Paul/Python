...
# Example 11-6
# Adds a set of features to an imported slideshow base class--note editing, a 
# delay scale and file label
...
SlideShow subclass which adds note files with an attached PyEdit object, a scale
for setting the slideshow delay interval, and a lable that gives the name of the 
image file currently being displayed;

Version 1.2 is a Python 3.x port, but also improves repackibng note for 
expansion when it's unhidden, catches note destroys in a subclass to avoid 
exceptions when popup window or full component editor has been closed, and runs 
update() before inserting text into newly packed note so it is positioned 
correctly at line 1 (see the book's coverage of PyEdit updates).
################################################################################
"""
...
class SlideShowPlus(SlideShow):
    ...
    def loadNote(self):
        if self.image and self.editorUp:
            ...
            noteDir = os.path.join(os.path.dirname(self.image[0]), 'notes')
            if not os.path.exists(noteDir):
                os.mkdir(noteDir)
            root = os.path.join(noteDir, os.path.split(root)[1])
            notefile = os.path.abspath(root + '_' + ext.replace('.', '') +'.note')
