...
# Example 11-8
# Is a tkinter canvas with keyboard and mouse event bindings to allow users to 
...
Uses time.sleep loops to implement object move loops, such that only one move 
can be in progress at once; this is smooth and fast, but see the widget.after
and thread-based subclasses here for other techniques. 
...
Keyboard commands:
    ...
    q = Select Object       x = Pick pen modes
    z = Save postscript     j = Save JPEG
    m = Save BMP            n = Save PNG
    g = Save GIF            ? = Help                
    other = clear text
"""
...
class MovingPics:
    ...
    def onOptions(self, event):
        keymap = {
            ...
            'f': MovingPics.fillObject,         # use unbound method object
            'b': MovingPics.fillBackground,     # else lambda passed self
            'p': MovingPics.addPhotoItem,
            'z': MovingPics.savePostscript,
            'm': lambda self: self.savePicture('.bmp'),
            'g': lambda self: self.savePicture('.gif'),
            'j': lambda self: self.savePicture('.jpg'),
            'n': lambda self: self.savePicture('.png'),
            '?': MovingPics.help,
            'q': lambda self: self.onSelect(event)
        }
        try:
    ...
    def savePicture(self, ext):
        file = asksaveasfilename()
        if file:
            self.setTextInfo('')
            self.canvas.update()
            imgpil = ImageConvert(file)
            imgpil.saveAs(file, ext, self.parent, self.canvas)
...
if __name__ == '__main__':
    ...
    if len(argv) == 2: PicDir = argv[1]         # '..' fails if run elsewere
