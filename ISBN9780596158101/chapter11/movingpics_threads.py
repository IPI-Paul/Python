...
# Example 11-10
# Shows how to achieve such parallelism with threads.
# Author: Mark Lutz
# Last modified: 

"""
PyDraw-threads: use threads to move objects; works okay on windows provided that 
canvas.update() not called by threads (else exits with fatal errors, some objs 
start moving immediately after drawn, etc.); at least some canvas method calls 
must be thread safe in tkinter; this is less smooth than time.sleep, and is 
dangerous in general: threads are best coded to updated global vars, not change 
GUI;
"""
...
class MovingPicsThreaded(MovingPics):
    ...
    def doMove(self, object, event):
        ...
        for i in range(reptX):
            ...
            # canvas.update()
            time.sleep(pickDelays[0])           # this canchange
