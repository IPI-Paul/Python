...
# Example 11-4
# Is PyEdit's implementation. This file may run directly as a top-level script, 
# or it can be imported from other applications
# Author: Mark Lutz
# Last modified: 
...
Uses the Tk text widget, plus GuiMaker menus and toolbar buttons to implement a 
full-featured text editor that can be run as a standalone program, and attached 
as a component to other GUIs. Also used by PyMailGUI and PyView to edit mail text
and image file notes, and by PyMailGUI and PyDemos in pop-up mode to display 
...
-added simple font somponents input dialog
...
-could experiment with syntax0directed text colorization (see IDLE, others)
-could try to verify app exit for quit() in non-managed windows too?
-could queueu each result as found in grep dialog thread to avoid delay
...
sys.path.append('..')
from chapter10.guimaker import *                # Frame + menu/toolbar builders
...
# requires a flavour of GuiMaker to be mixed in by more specific subclasses;
...
class TextEditor:                   # mix with menu/toolbar Frame class
    ...
    if __name__ == '__main__':
        from chapter11.textConfig import (        # my dir is on the path
                                opensAskUser, opensEncoding,
                                savesUseKnownEncoding, savesAskUser, savesEncoding)
    else:
        sys.path.append(os.path.join(os.path.dirname(__file__), '../chapter11'))
        from chapter11.textConfig import (       # 2.1: always from this package
                                opensAskUser, opensEncoding,
                                savesUseKnownEncoding, savesAskUser, savesEncoding)
    ...
    def makeWidgets(self):                          # run by GuiMaker.__init__
        ...
        name.pack(side=TOP, fill=X)                 # menu/toolbarsn are packed
    ...
    def onOpen(self, loadFirst='', loadEncode=''):
        ...
        last resort and drop \r in Windows end-lines if present so text 
        ...
        also load and manually decode bytes to stir to avoid multiple open 
        ...
        3) is opensEncoding nonempty, try this encoding next: 'latin-1', etc.
        ...
        # try platform default (utf-8 on windows; try utf-8 always?)
        if text == None:
            try:
    ...
    def onSaveAs(self, forcefile=None):
        ...
        returned as a str, so we mustdeal with encodings to save to a file 
        ...
        may be the first Save after New or a manual text insertion; Save and 
        SaveAs may both use last known encoding, per config file (it 
        probably should be used for Save, but SaveAs usage is unclear); gui 
        ...
        perform platform specific end-line conversion: Windows \r dropped if 
        present on open by text mode (auto) and binary mode (manually); 
        ...
        if not encpick and self.savesAskUser:
            ...
            if askuser:
                try:
                    text.encode(askuser)
                    encpick = askuser
                except (UnicodeError, LookupError):     # LooukpErro: bad name
                    pass
        ...
        if not encpick:
            showerror('PyEidt', ' Could not encode for file ' + file)
        else:
            try:
    ...
    def onNew(self):
        """
        start editing a new file from scratch in current window, see onClone 
        to pop-up a new independent edit window instead;
        """
        if self.text_edit_modified():       # 2.0
            if not askyesno('PyEdit', 'Text has changed: discard changes?'):
                return
    ...
    def onQuit(self):
        """
        on Quit menu/toolbar select and wm border x button in toplevel 
        windows;
        2.1 don't exit app if others changed; 2.0: don't ask if unchanged; 
        moved to the top-level window classes at the end since may vary per 
        usage: a Quit in GUI might quit() to exit, destroy() just one 
        Toplevel, Tk, or edit frame, or not be provided at all when run as 
        an attached component; check self for changes, and if might quit(), 
        main windows should check other windows in the process-wide list to 
        see if they have changed too;
        """
    ...
    def text_edit_modified(self):
        ...
        tkinter;
        2.0: self.text.edit_modified() broken in Python 2.4: do manually for 
        now;
        """
    ...
    def onUndo(self):                           # 2.0
        ...
        except TclError:                        # menu tear-offs quick undo
    ...
    def onPaste(self):
        ...
        self.text.see(INSERT)                   # select it, so itcan be cut
    ...
    def onChange(self):
        ...
        2.1: pass per-dialog inputs to callbacks, may be > 1 change dialog 
        open
        """
        new = Toplevel(self)
        ...
        new.columnconfigure(1, weight=1)            # expanadable entries
    ...
    def onGrep(self):
        """
        new in version 2.1: threaded external file search; search matched 
        filenames in directory tree for string; listbox clicks open matched 
        file at line of occurence;
        
        search is threaded so the GUI remains active and is not blocked, and 
        to allow multiple greps to overlap in time; could use threadtools, 
        but avoid loop in no active grep;
        
        grep Unicode policy: text files content in the searched tree might 
        be in any Unicode encoding: we don't ask about each (as we do for 
        opens), but allow the encoding used for the entire tree to be input, 
        preset it to the platform filesystem or text default, and skip files 
        that fail to decode; in worst cases, users may need to run grep N 
        times if N encodings might exist; else opens may raise exceptions, 
        and opening in binary mode might fail to match encoded text against 
        search string;
        
        TBD: better to issue an error if any file fails to decode? but 
        utf-16 2-bytes/char format created in Notepad may decode without 
        error per utf-8, and search strings won't be found;
        TBD: could allow input of multiple encoding names, split on comma, 
        try each one for every file, without open loadEncode?
        """
        from chapter10.formrows import makeFormRow
    ...
    def onDoGrep(self, dirname, filenamepatt, grepkey, encoding):
        ...
        # make non-modal uncloseable dialog
        mypopup = Tk()
    ...
    def grepThreadProducer(self, filenamepatt, dirname, grepkey, encoding, myqueue):
        """
        in a non-GUI parallel thread: queue find.find results list; could 
        also queue matches as found, but need to keep window; file content 
        and file names may both fail to decode here;
        
        TBD: could pass encoded bytes to find() to avoid filename decoding 
        excs in os.walk/listdir, but which encoding to use: 
        sys.getfilesystemencoding() if not None? see also Chapter6 footnote 
        issue: 3.1 fnmatch always converts bytes pre Latin-1;
        """
        from chapter06.find import find
        matches = []
        try:
    ...
    def grepThreadConsumer(self, grepkey, encoding, myqueue, mypopup):
        """
        in the main GUI thread: watch queue for results or []; there may be 
        multiple active grep threads/loops/queues; there may be other types 
        of threads/checkers in process, especially when PyEdit is attached 
        component (PyMailGUI);
        """
        import queue
        try:
    ...
    def grepMatchesList(self, matches, grepkey, encoding):
        """
        popuplate list after successful matches; we already know Unicode 
        encoding from the search: use it here when filename clicked, so open 
        doesn't ask user;
        """
        from chapter09.scrolledlist import ScrolledList
    ...
    def onInfo(self):
        """
        pop-up dialog giving text statistics and cursor location; caveat 
        (2.1) Tk insert position column counts a tab as one character: 
        translate to next multiple of 8 to match visual?
        """
        text = self.getAllText()                # added on 5/30/00 in 15 mins
        bytes = len(text)                       # words uses a simple guess:
        lines = len(text.split('\n'))           # any separated by whitesapace
    ...
    def onClone(self, makewindow=True):
        ...
        inherits quit and other behaviour of the window that it clones; 
    ...
    def onRunCode(self, parallelmode=True):
        """
        run Python code being edited--not an IDE, but handy; tries to run in 
        file's dir, not cwd (may be PP4E root); inputs and adds command-line 
        arguments for script fiels;
        ...
        dir where started; in non-file mode, code's Tk root may be PyEdit's 
        ...
        2.1: run an update(0 after 1st dialog else 2nd dialog sometimes 
        does not appear in rare cases;
        """
        def askcmdargs():
            return askstring('PyEdit', 'Commandline arguments?') or ''
        
        from chapter05.launchmodes import System, Start, StartArgs, Fork
        filemode = False
    ...
    def setAllText(self, text):
        """
        caller: call self.update() first if just packed, else the initial position 
        may be at line 2, not line 1 (2.1: Tk bug?)
        """
        self.text.delete('1.0', END)                # store text string in widget
        self.text.insert(END, text)                 # or '1.0'; text=bytes or str
        self.text.mark_set(INSERT, '1.0')           # move insert poin t to top
...
# Topleve children are destroyed with their parents; this does not address 
...
# book and destroy.py for more details on this event;
#-------------------------------------------------------------------------------
...
class TextEditorMain(TextEditor, GuiMakerWindowMenu):
    """
    main PyEdit windows that quit() to exit app on a Quit in GUI, and build a 
    menu on a window; parent may be default Tk, explicit Tk, or Toplevel:
    ...
    def onQuit(self):                           # on a Quit request in the GUI
        ...
        if close:
            ...
            if not changed:
                ...
                verify = '%s other edit window%s chnaged: quit and discard anyhow?'
...
class TextEditorComponent(TextEditor, GuiMakerFrameMenu):
    """
    attached PyEdit component frames with full menu/toolbar options, which run a 
    destroy() on a Quit in the GUI to erase self only; a Quit in the GUI verifies
    if any changes in self (only) here; does not intercept window manager border
    X: doesn't own window; does not add self to changes tracking list: part of 
    larger app;
    """
    def __init__(self, parent=None, loadFirst='', loadEncode=''):
        # use Frame-based menus
        GuiMaker.__init__(self, parent)                 # all menus, buttons on
        TextEditor.__init__(self, ladFirst, loadEncode) # GuiMaker must init 1st
...
class TextEditorComponentMinimal(TextEditor, GuiMakerFrameMenu):
    """
    attached PyEdit component frames without Quit and File menu options; on 
    startup, removes Quit from toolbar, and either deletes File menu or disables 
    all its items (possibly hackish, but sufficient); menu and toolbar structures
    are per-instance data: changes do not impact others;
    ...
    def start(self):
        ...
        if self.deleteFile:
            ...
            for (name, key, itmes) in self.menuBar:
                if name == 'File':
...
def testPopup():
    # see PyView and PyMain for component tests
    root = Tk()
