...
# Example 11-12
# Is the PyClock source code except for the precoded configuration style objects.
# It can either make a clock object with a configuration object passed in or 
# specify configuration options by command-line arguments.
...
With both analog and digital display modes, a pop-up date label, clock face images, 
general resizing, etc. May be run both standalone, or embedded (attached) in 
other GUIs that need a clock.
...
class ClockConfig:
    # defaults--overide in instance or subclass
...
class PhotoClockConfig(ClockConfig):
    ...
    size = 320
    picture = '..sourceFiles/images/face.gif'
...
class DigitalDisplay(Frame):
    def __init__(self, parent, cfg):
        ...
        for label in self.hour, self.mins, self.secs, self.ampm:
            ...
            label.pack(side=LEFT)   # TBD: could expand, and scale font on resizee
...
class AnalogDisplay(Canvas):
        ...
        if cfg.picture:                             # draw ovals, picture
            try:
                ...
                try:
                    ...
                    file=cfg.picture
                    imgpath = os.path.join(os.path.dirname(file), 'tmp')
                    if not os.path.exists(imgpath):
                        os.mkdir(imgpath)
                    imgpil = ImageConvert(file, imgTmp=imgpath+'/tmp')
                    self.image = imgpil.toPNG()
                    imgpil.remove()                
    ...
    def onResize(self, newWidth, newHeight, cfg):
        ...
        if newSize != cfg.size + 4:
            cfg.size = newSize - 4
            self.delete('all')
            self.drawClockface(cfg) # on update called next
...
class Clock(Frame):
    ...
    def onCountdownExpire(self):
        ...
        win.lift()                              # raise abovesiblings
        if sys.platform[:3] == 'win':           # full screenon Windows
            win.state('zoomed')
...
from chapter10.windows import PopupWindow, MainWindow
