...
# Example 9-25
# Adds another row to display column sums and adds buttons to clear all fields 
# to zero and calculate column sums
# Author: Mark Lutz
# Last modified: 
...
def onSum():
    ...
    for i in range(numcol):
        for j in range(numrow):
            if not ('__import__' in rows[j][i].get() and \
               'system' in rows[j][i].get()):
