...
# Example 13-23
# This module provides a convenient interface that combines standard library 
# tools we've already met in this chapter--the email package to compose messages 
# with attachments and encodings, and the smtplib module to send the resulting email text
...
import email.utils, email.encoders              # data string, base64
...
from email.mime.multipart import MIMEMultipart  # type-specific meassages
...
def fix_encode_base64(msgobj):
    """
    workaround for a genuine bug in Python 3.1 email package that prevents mail 
    text generation for binary parts encoded with base64 or other email encodings; 
    the normal email.encoder run by the constructor leaves payload as bytes, even
    though it's encoded to base64 text form; this breaks email text generation 
    which assumes this is text and requires it to be str; net effect is that only
    simple text part emails can be composed in Py 3.1 email package as is - any 
    MIME-encoded binary part cause mail text generation to fail; this bug seems 
    likely to go away in a future python and email package, in which case this 
    should become a no-op; see Chapter 13 for more details;
    """
...
def fix_text_required(encodingname):
    ...
    to the different types for different Unicode emcodings in Python 3.1 due to
    the different ways it MIME-encodes some types of text; see Chapter 13; the 
    only other alternative is using generic Message and repeating much code;
    """
...
class MailSender(MailTool):
    """
    send mail: format a message, interface with an SMTP server; works on any 
    machine with Python+Inet, doesn't use cmdline mail; a nonauthenticating 
    client: see MailSenderAuth if login required;
    4E: tracesize is num chars of msg text trace: 0=none, big=all;
    ...
    def sendMessage(self, From, To, Subj, extrahdrs, bodytext, attaches,
        ...
        bodytext is main text part, attaches is list of filenames, extrahdrs is 
        list of (name, value) tuples to be added; 
        ...
        decoded addresses (possibly in full name+<add> forma); client must parse
        to split theses on delimiters, or use multiline input;
        ...
        # 4E assume main body text is already in dessired encoding;
        ...
        # 4E: non-ASCII hdrs encoded on ends; encode just name in addres, else
        # smtp may drop the message completely; encodes all envelope To names
        # (but not addr) also, and assumes servers will allow;
    ...
    def addAttachments(self, mainmsg, bodytext, attaches,
        ...
        for (filename, fileencode) in zip(attaches, encodings):
            ...
            if maintype == 'text':                      # 4E: text needs encoding
                ...
                msg = MIMEImage(
                    data.read(), _subtype=subtype)  # , _encode=fix_encode_base64)
                data.close()
            elif maintype == 'audio':
                data = open(filename, 'rb')
                msg = MIMEAudio(
                ...
                # email.encoder.encode_base64(msg)      # encode using base64
            
            # set filename and attache to container
    ...
    def encodeHeader(self, headertext, unicodeencoding='utf-8'):
        """
        4E: encode composed non-ascii message headers content per both email and
        Unicode standards, according to an optional user setting or UTF-8;
    ...
    def encodeAddrHeader(self, headertext, unicodeencoding='utf-8'):
        ...
        may run into errors when it tries to decode the entire mail as ASCII;
        ...
        try:
            ...
            for name, addr in pairs:
                ...
                except:                         # else try to encode name part
                    try:
...
class MailSenderAuth(MailSender):
    ...
    def __init__(self, tracesize=256, smtpserver=None, smtpuser=None):
        MailSender.__init__(self, smtpserver, tracesize=tracesize)
    ...
    def askSmtpPassword(self):
        assert False, 'Subclass mst define method'
