...
# Example 13-4
# A utility that always transfers in binary mode with retrbinary
# Author: Mark Lutz, Paul I Ighofose
# Last modified: 2019-02-07

"""
Fetch an arbitrary file by FTP. Anonymous FTP unless you pass a user=(name, pswd)
tuple. Self-test FTPs a test file and site.
"""

from ftplib import FTP                  # socket-based FTP tools
from os.path import exists, getsize     # file existence test
...
def getfile(file, site, dir, user=(), *, verbose=True, refetch=False):
    ...
    """
    if exists(file) and not getsize(file) == 0 and not refetch:
