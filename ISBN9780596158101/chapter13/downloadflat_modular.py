...
# Example 13-12
# Shows one way to refactor (reorganize) the download script. By wrapping its 
# parts in functions, they become reusable in other modules, including our 
# upload program
# Author: Mark Lutz
# Last modified: 
...
"""
################################################################################
use FTP to copy (download) all files from a remote site and directory to a 
directory on the local machine; this version works the same, but has been
refactored to wrap up its code in functions that can be reused by the uploader,
and possibly other programs in the future - else code redundancy, whcih may make 
the two diverge over time, and can double maintenance costs.
################################################################################
"""
...
defaultRdir = (len(sys.argv) > 1 and sys.argv[1]) or '.'
defaultUser = input('User Name: ')
...
def configTransfer(site=defaultSite, rdir=defaultRdir, user=defaultUser):
    ...
    cf.remoteuser = user
    cf.localdir = (len(sys.argv) > 2 and sys.argv[2]) or '.'
...
def isTextKind(remotename, trace=True):
    ...
    for unknowns,   quess may be (None, None): binary
