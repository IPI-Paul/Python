...
# Example 13-14
# Refactors the FTP code one more time in order to leverage Python's class feature
# Author: Mark Lutz
# Last modified: 

"""
################################################################################
use FTP to download or upload all files in a single directory from/to a remote 
site and directory; this version has been refactored to use classes and OOP for 
namespace and a natural structure; we could also structure this as a download 
superclass, and an upload subclass which redefines the clean and transfer 
methods, but then there is no easy way for another client to invoke both an 
upload and download; for the uploadall variant and possibly others, also make 
single file upload/download code in orig loops methods;
################################################################################
"""
...
dfltSite = 'localhost'
dfltRdir = (len(sys.argv) > 3 and sys.argv[3]) or '.'
dfltUser = input('User Name: ')

class FtpTools:
    # allow these 3 to be redefined
    def getlocaldir(self):
        return (len(sys.argv) > 3 and sys.argv[3]) or '.'
    ...
    def downloadOne(self, remotename, localpath):
        ...
        local name nedd not be same as remote name
        """
...
if __name__ == '__main__':
    ftp = FtpTools()
    xfermode = 'download'
    rsite = (len(sys.argv) > 2 and sys.argv[2]) or 'localhost'
    if len(sys.argv) > 1:
        xfermode = sys.argv.pop(1)      # get+del 2nd arg
    if xfermode == 'download':
        ftp.configTransfer(site=rsite)
        ...
        print('Usage: ftptools.py ["download" | "upload"] [remotesite] [remotedir] [localdir]')
