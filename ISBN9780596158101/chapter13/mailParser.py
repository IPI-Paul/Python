...
# Example 13-25
# Implements the last major class in the mailtools package--given the (already 
# decoded) text of an email mesage, its tolls parse the mail's content into a 
# message object, with headers and decoded parts
# Author: Mark Lutz
# Last modified: 
...
class MailParser(MailTool):
    ...
    subtle thing: Message object payloads are either a simple string for 
    non-multipart messages, or a list of Message objects if multipart (possibly
    nested); we don't need to distinguish between the two cases here, because 
    the Message walk generator first, and so works fine on non-multipart messages
    too (a single object is walked);
    
    for simple messages, the message body is always considered here to be the 
    sole part of the mail; for multipart messages, the parts list includes the 
    main message text, as well as all attachments; this allows simple messages 
    not of type text to be handled like attachments in a UI (e.g., saved, opened);
    Message payload may also be None for some oddball part types;
    
    4E note: in Py 3.1, text part payloads are returned as bytes for decode=1, 
    and might be str otherwise; in mailtools, text is stored as bytes for file 
    saves, but main-text bytes payloads are decoded to Unicode str per mail 
    header info or platfom default+guess; cl8ents may need to convert other 
    payloads: 
    PyMailGUI uses headers to decode parts saved to binary files;
    
    4E supports fetched message header auto-deconding per its own content, both 
    for general headers such as Subject, as well as for names in address header 
    such as From and To; client must request this after parse, before display:
    parser doesn't decode;
    """
    
    def walkNamedParts(self, message):
        """
        generator to avoid repeating part naming logic; skips multipart headers, 
        makes part filenames; message is already parsed email.message.Message 
        object; doesn't skip oddball types: payload may be None, must handle in 
        part saves; some others may warrant skips too;
        """
    ...
    def partName(self, part, ix):
        ...
        filename: tries Content-Disposition, then Content-Type name param, or 
        generates one based on mimetype guess;
        """
    ...
    def saveParts(self, savedir, message):
        ...
        returns [('maintype/subtype', 'filename')] list for use by callers, but 
        does not open any parts or attachments here;
        get_payload decodes base64, quoted-printable, uunecoded data;
        mail parser may give us a None payload for oddball types we probably 
        should skip over: convert to str here to be safe;
        """
    ...
    def partsList(self, message):
        ...
        using same filename logic as saveParts, but bo not store the part files
        here
        """
    ...
    def findOnePart(self, partname, message):
        ...
        intended to be used in cojnunction with parts List;
        we could also use mimetypes.guess_type(partname) here;
    ...
    def decodedPayload(self, part, asStr=True):
        """
        4E: decode text part bytes to Unicode str for display, line wrap, etc.; 
        part is a Message; (decode=1) undoes MIME email encodings (base64, 
        uunecode, qp), bytes.decode() performs additional Unicode text string 
        decodings; tries charset encoding name in message headers first (if 
        present, and accurate), then tries platform defaults and a few guesses 
        before giving up with error string
        """
    ...
    def findMainText(self, message, asStr=True):
        """
        for text-oriented clients, return first text part's str; for the payload
        of a simple message, or all parts of a multipart message, looks for 
        text/plain , then text/html, then text/*, before deducing that there is 
        no text to display; this is a heuristic, but covers most simple,
        multipart/alternative, and multipart/mixed messages; content-type
        defaults to text/plain if not in simple msg;
        
        handles message nesting at top level by walking instead of list scans;
        if non-multipart bt type is text/html, returns the HTML as the text
        with an HTML type: caller may open in web browser, extract plain text, 
        etc; if nonmultipart and not text, there is no text to display; save/open
        message content in UI; caveat: does not try to concatenate multiple 
        inline text/plain parts if any;
        4E: text payloads may be bytes--decodes or str here;
    ...
    def decodeHeader(self, rawheader):
        """
        4E: decode existing i18n mesage header text per both email and Unicode
        standards, accordning to its content, return as isif unencoded or fails;
        clien must call this to display: pased Message object does not decode;
        ...
        return multiple parts if any substrings of hdr are encoded, and returns 
        all bytes in parts list if any encodings found (with unencoded part encoded
        as raw-unicode-escape and enc=None) but returns a single part with enc=None
        ...
        the following first atempt code was okay unless any encoded subsrings, or 
        ...
        """
        try:
            parts = email.header.ecode_header(rawheader)
            decoded = []
    ...
    def decodeAddrHeader(self, rawheader):
        """
        4E: decode exisiting i18n address header text per email and unicode,
        according to its content; must parse out first part of email address to
        get i18n part: '"=?UTF-8?Q?Walmart?=" <newsletters@walmat.com';
        From will probably have just 1 addr, but To, Cc, Bcc may have many;
        
        decodeHeder handles nested encode substrings within an entire hdr, but 
        we can't simply call it for entire hdr here because it fails if encoded
        name substring ends in " quote instead of whitespace or endstr; see also
        encodeAddrHeader in mailSender module for inverse of this;
    ...
    def splitAddresses(self, field):
        """
        4E: use comma separator for multiple addrs in the UI, and getaddresses 
        to split correctly and allow for comma in the name parts of addresses;
        used by PyMailGUI to split To, Cc, Bcc as needed for user inputs and 
        copied headers; returns empty list if field is empty, or any exception
        occurs;
        """
        try:
    ...
    def parseMessage(self, fulltext):
        ...
        the call here same as calling email.messge_from_string()
        """
        try:
