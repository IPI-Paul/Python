...
# Example 13-20
# Implements an interactive email session--users may type commands to read, send, 
...
"""
################################################################################
pymail - a simple console email interface client in Python; uses Python poplib
module to view POP email messages, smtplib to send new mails, and the email 
package to extract mail headers and payload and compose mails;
################################################################################
"""

import poplib, smtplib, email.utils, mailconfig, sys, _thread as thread, time
...
if input('Do you want to start the localhost POP3 Server? ') in ['y', 'Y']:
    ...
    usingLocalPOP = True
    time.sleep(2)
elif input('Are you using LocalPop mail server? ') in ['y', 'Y']:
    usingLocalPOP = True
...
def decodeToUnicode(messageBytes, fetchEncoding=fetchEncoding):
    ...
    use global setting (or by platform default, hdrs inspection, intelligent 
    guess); in Python 3.2/3.3, this step may not be required: if so, return 
    message intact;
    """
    return [line
            .replace(b'\\n', b'\n')
            .replace(b'\\r', b'\r')[2:-1]
            .decode(fetchEncoding) for line in messageBytes]    
...
def sendmessage():
    ...
    try:
        failed = server.sendmail(From, To, str(msg))    # may also require exc
    except:
    ...
    if verify and input('Delete? ')[:1] not in ['y', 'Y']:
        ...
        try:
            ...
            for msgnum in toDelete:             # reconnect to delete mail
                if usingLocalPOP:
                    #server.dele(str(msgnum).encode().join(delList[str(msgnum)]))
...
def showmessage(i, msgList):
    if 1 <= i <= len(msgList):
        ...
        content = msg.get_payload()         # prints paylooaad: string, or [Messages]
...
def interact(msgList, mailfile):
    ...
    while True:
        ...
        # mail
        elif command[0] in ['m', 'M']:              # send a new mail via SMTP
            sendmessage()
            # excfile('smtpmail.py', {})            #alt: run file in own namespace
