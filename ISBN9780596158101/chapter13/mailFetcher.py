...
# Example 13-24
# Does the work of interfacing with a POP email server--loading deleting, and
# synhronizing
...
"""
################################################################################
retrieve, delete, match mail from POP server (see __init__ for docs, test)
################################################################################
"""
...
class MailFetcher(MailTool):
    """
    fetch mail: connect, fetch headers+mail, delete mail
    ...
    #use setting in client's mailconfig on import earch path;
    ...
    def decodeFullText(self, messageBytes):
        ...
        an 8-bit encoding such as latin-1 will likley suffice for most emails, as 
        ...
        for kind in kinds:                      # may cause mail saves to fail
            try:
                text = [line
                        ...
                        .replace(b'\\r', b'\r')[2:-1]
                        .decode(kind) 
                        for line in messageBytes]
                break
            except (UnicodeError, LookupError): # LookupError: bad name
                pass
        if text == None:
            # try returning headers + erro msg, else except may kill client;
            ...
            for common in commons:
                try:
                    text = [line
                            .replace(b'\\n', b'\n')
                            .replace(b'\\r', b'\r')[2:-1]
                            .decode(common) for line in hdrsonly]
                    break
                except UnicodeError:
                    pass
            else:                                               # none worked
                try:
                    text = [line
                            .replace(b'\\n', b'\n')
                            .replace(b'\\r', b'\r')[2:-1]
                            .decode() for line in hdrsonly] # platform dflt?
                except UnicodeError:
    ...
    def downloadMessage(self, msgnum):
        """
        load full raw text of one mail msg, given its POP relative msgnum; caller 
        must parse content
        """
        ...
        try:
            ...
            server.quit()
        msglines = self.decodeFullText([line for line in msglines]
                                       )            # raw bytes to Unicode str
    ...
    def downloadAllHeaders(self, progress=None, loadfrom=1):
        ...
        4E: add mailconfig.fetchlimit to support large email inboxes: if not None,
        only fetches that many headers, and returns others as dummy/empty mail;
        else inboxes like one of mine (4K emails) are not practical to use;
        4E: pass loadfrom along to downloadAllMsgs (a buglet);
        """
        if not self.srvrHasTop:                     # not all servers support TOP
            # niavely load full msg text
    ...
    def downloadAllMessages(self, progress=None, loadfrom=1):
        """
        load full message text for all msgs from loadfrom..n, despite any caching
        that may be being done in the caller; much slower than downloadAllHeaders, 
        if just need hdrs;
        
        4E: support mailconfig.fetchlimit: see downloadAllHeaders; could use 
        server.list() to get sizes of skipped emails here too, but clients 
        probably don't care about these anyhow;
        """
    ...
    def deleteMessages(self, msgnums, progress=None, delList=None):
        """
        delete multiple msgs off server; assumes email inbox unchanged since 
        msgnums sere last deterined/loaded; use if msg headers not avaiable as state 
        information; fast, but poss dangerous: see deleteMessagesSafely
        """
        self.trace('deleting mail')
        server = self.connect()
        try:
            for (ix, msgnum) in enumerate(msgnums): # don't reconnect for each
                if progress: progress(ix+1, len(msgnums))
                if delList:
                    server.dele(str(msgnum) + ' ' + (delList[msgnum]))
                else:
                    server.dele(msgnum)
        finally:
            server.quit()                           # changes msgnums: reload
    
    def deleteMessagesSafely(self, msgnums, synchHeaders, progress=None):
        """
        delete multiple mss off server, but use TOP feches to check for a match 
        on each msg's header part before deleting; sumes the email server supports
        the TOP interface of POP, else raises TopNotSupported - client may call 
        deleteMessages;
        
        use if the mail server might change the inbox since the email index was
        last fetched, thereby changing POP relative message numbers; this can
        happen if email is deleted in a different client; some ISPs may also 
        move a mail from inbox to the undeliverable box in response to a failed 
        download;
        
        synchHeaders must be a list of already loaded mail hdrs text, corresponding 
        to selected msgnums (requires state); raises exception if any out of synch
        with the email server; inbox is locked until quit, so it should not change 
        between TOP check and actual delete: synch check must occur here, not in 
        caller; may be enough to call checkSynchrror+deleteMessages, but check
        ...
        errmsg = 'Message %s out of synsch with server.\n'
            ...
            for (ix, msgnum) in enumerate(msgnums):
                ...
                if not self.headersMatch(msghdrs, synchHeaders[msgnum-1]):
                    ...
                    try:
                        ...
                        filename = MailParser().parseMessage(msghdrs)['File']
                        server.dele(str(msgnum) + ' ' + filename)
        finally:                                    # changes msgnums: reload
            server.quit()
    
    def checkSynchError(self, synchHeaders):
        """
        check to see if already loaded hdrs ext in sycnhHeaders list matches 
        what is on the server, using the TOP command in POP to fetch headers text;
        use if inbox can change due to deletes in other client, or automatic 
        action by email server; raises except if out of synch, or error while 
        talking to server;
        
        for speed, only checks last in last: this catches inbox deletes, but 
        assumes server won't insert before last (true for incoming mail); check 
        inbox size first: smaller if just deletes; else top will differ if deletes
        and newly arrived messages added at end; result valid only when run: inbox 
        may change after return;
        """
        ...
        try:
            ...
            if self.srvrHasTop:
                resp, hdrlines, respsz = server.top(lastmsgnum, 0) # hdr only
                hdrlines = self.decodeFullText(hdrlines)
                lastmsghdrs = '\n'.join(hdrlines)
    ...
    def headersMatch(self, hdrtext1, hdrtext2):
        """
        may not be as simple as a string compare: some servers add a "Status:" 
        header that changes over time: on one ISP, it begins as "Status: U" 
        (unread), and changes to "Satus:RO" (read, old) after fetched once - 
        throws off synch tests if new when index fetched, but have been fetched 
        once before delete or last-message check; "Message-id:" line is unique
        per message in theory, but optional, and can be anything if forged; 
        match more common: try first; parsing costly: try last
        """
    ...
    def askPopPassword(self):
        assert False, 'Subclass must define mehtod'
