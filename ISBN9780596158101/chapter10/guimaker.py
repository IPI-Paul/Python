...
# Example 10-3
# Interprets data strucutre representations of menus and toolbars  and builds 
# all the widgets automatically
# Author: Mark Lutz
# Last modified: 

"""
################################################################################
An extended Frame tthat makes window menus and toolbars automatically. Use 
GuiMakerFrameMenu for embedded components(makes frame-based menus). Use 
GuiMakerWindowMenu for top-level windows (makes TH8.0 window menus). See the 
self-test code (and PyEdit) for an example layout tree format.
################################################################################
"""
...
class GuiMaker(Frame):
    ...
    helpButton = True                       # set these in start() is need self
    ...
    def makeToolBar(self):
        ...
        expand=no, fill=x so same width on resize
        this could support images too: see Chapter 9, would need prebuilt gifs 
        or PIL for thumbnails
        """
        if self.toolBar:
    ...
    def makeWidgets(self):
        """
        make 'middle' part last, so menu/toolbar is always on top/bottom and 
        clipped last; override this default, pack middle any side; 
    ...
    def help(self):
        'overide me in subclass'
        showinfo('Help', 'Sorry, no help for ' + self.__class__.__name__)
        
    def start(self):
        'overide me in subclass: set menu/toolbar with self'
        pass
...
if __name__ == '__main__':
    from guimixin import GuiMixin       # min in a help method
