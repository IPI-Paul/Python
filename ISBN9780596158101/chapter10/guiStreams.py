...
# Example 10-12
# Arranges to map input and output sources to pop-up windows in a GUI 
# application
# Author: Mark Lutz
# Last modified: 
...
"""
################################################################################
first-cut implementation of file-like classes that can be used to redirect input
and output streams to GUI displays; as is, input comes from a common dialog 
pop-up (a single output+input interface or a persistent Entry field for input 
would be better); this also does not properly span lines for read requests with 
a byte count > len(line); could also add __iter__/__next__ to GuiInput to support 
line iterationlike files but would be too many popups
################################################################################
"""
...
if __name__ == '__main__':                  # self test when run
    ...
    Button(root, text='test streams', 
           command=redirectedGuiFunc(makeUpper)).pack(fill=X)
