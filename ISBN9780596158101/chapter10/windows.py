...
# Example 10-16
# Provides wrapper classes for the most common window types--a main application 
...
Allows same GUIto be main, pop-up, or attached; content classes may inherit from 
these directly, or be mixed together with them per usage mode; may also be called
directly without a subclass; designed to be mixed in after (further to the right 
than) app-specific classes; else, subclass gets methods here (destroy, 
okayToQuit), instead of from app-specific classes--can;t redefine.
################################################################################
"""
...
class _window:
    ...
    iconpatt = '../sourceFiles/images/*.ico'            # may be reset
    iconmine = 'py.ico'
        ...
        if iconfile:
            try:
                ...
                if sys.platform[:3] == 'cyg':
                    self.iconphoto(True, PhotoImage(file=iconfile.replace('ico', 'png')))
                else:
                    pass
    ...
    def findIcon(self):
        ...
        _window.foundicon = iconfile               # don't search argain
        return iconfile
...
class PopupWindow(Toplevel, _window):
    ...
    def quit(self):                                         # redef me to change
        if askyesno(self.__app, 'Verify Quit Window?'):     # or calldestroy
