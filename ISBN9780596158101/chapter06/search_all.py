...
# Example 6-17
# Employs the following standard Python tools that were met in preceeding chapters:
# os.walk to visit files in a directory, os.path.splitext to skip over files with
# binary-type extensions, and os.path.join to portably combine directory path 
# and filename
# Author: Mark Lutz
# Last modified: 
...
Search all files at and below a named directory for a string; uses the os.walk 
interface, rather than doing a find.find to collect names first; similar to 
calling visitfile for each find.find result for '*' pattern;
################################################################################
"""
...
if __name__ == '__main__':
    if len(sys.argv) > 2:
        ...
        print('Usage: search_all.py dir SearchString')