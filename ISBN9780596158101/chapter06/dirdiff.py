...
# Example 6-11
# Codes and debugs a single directory comparison of filenames
# Author: Mark Lutz
# Last modified: 
...
Compare two directories to find files that exist in one but not the other. This 
version useds the os.litdir function and list difference. Note that this script 
checks only filenames, not file contents--see difall.py for an enxtension that 
...
def difference(seq1, seq2):
    ...
    a set(seq1) - set(seq2) would work too, but sets are randomly ordered, so any 
    platform-dependent directory order would be lost
    """
    return [item for item in seq1 if item not in seq2]

def comparedirs(dir1, dir2, files1=None, files2=None):
    """
    Compare directory contents, but not actual files; may need bytes listdir arg 
    for undecodable filenames on some platforms
    """
