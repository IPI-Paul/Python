...
# Example 6-10
# Recursively copies a directory tree skipping past files that cause copy errors
# Author: Mark Lutz
# Last modified: 
...
Recursive copy of a directory tree. Works like a 'cp -r dirFrom/* dirTo' Unix 
command, and assumes that dirFrom and dirTo are both directories. Was written to 
get around fatal error messages under Windows drag-and-drop copies (the first 
bad file ends the entire copy operation immediately), but also allows for coding
more customised copy operations in Python.
################################################################################
"""
...
def copyfile(pathFrom, pathTo, maxfileload= maxfileload):
    ...
    if os.path.getsize(pathFrom) <= maxfileload:
        ...
        while True:
            ...
            if not bytesFom: break              # empty after last chunk
            fileTo.write(bytesFrom)

def copytree(dirFrom, dirTo, verbose=0):
    """
    Copy contents of dirFrom and below to dirTo, return (files, dirs) counts; 
    may need to use bytes for dirnames if undecodable on other platforms; may 
    need to do more file type checking on Unix: skip links, fifos, etc
    """
    fcount = dcount = 0
...
def getargs():
    ...
    try:
        ...
        if not os.path.isdir(dirFrom):
            ...
            if same:
                print('Error: dirFrom sames as dirTo')
            else:
                return (dirFrom, dirTo)
