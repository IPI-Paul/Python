...
# Example 16-6
# Is actioned when the main page View link is pressed
...
this could almost be an HTML file because there are likely no input paramsa yet,
...
<form method="post" action="%sonViewPswdSubmit.py">
    ...
    <p>
        <input name="lpop" type="checkbox" /> 
        Tick if you are you using LocalPop POP3 server!
    </p>
</form>
<hr />
<p>
    <i>Security note</i>: The password you enter above will be transmitted over 
    the Internet to the server machine, but is not displayed, is never transmitted
    in combination with a username unless it is encrypted or obfuscated, and is 
    never stored anywhere: not on the server (it is only passed along as hidden
    fields in susequent pages), and not on the client (no cookies are generated).
    This is still not guaranteed to be totally safe; use your browser's back 
    button to back out of PyMailCgi at any time.
</p>
"""
