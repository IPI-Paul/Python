...
# Example 16-13
# Uses function definitions nested in if statements to generate the selected 
...
"""
################################################################################
PyMailCGI encodes the POP password whenever it is sent to/from client over the 
Net with a username, as hidden text fields or explicit URL params; uses 
...
doesn't save the password on the server, and doesn't echo pswd as typed, but this
isn't 100% sae--this module file itself might be vulnerable; HTTPS may be better
and simpler but Python we server classes don't support;
################################################################################
"""
...
if not forceReadablePassword:
    ############################################################################
    # don't do anything by default: the urllib.parse.quote or cgi.escape calls 
    # in commonhtml.py will escape the password as needed to embed in URL or 
    # HTML; the cgi module undoes escapes automatically for us;
    ############################################################################
    ...
    ############################################################################
    # convert encoded string to/from a string of digit chars, to avoid problems 
    # with some special/nonprintable chars, but still leave the result 
    # semi-reusable (but encrypted); some browsers had problems with escaped 
    # ampersands, etc.;
    ############################################################################
...
if useCrypto:
    ############################################################################
    # use third-party pycrypto package's AES algoritm 
    ...
    mkey = 'pymailcgi3'.ljust(16, '-')      # key must be 16, 24, 0r 32 bytes
    
    def do_encode(pswd):
        over = len(pswd) % 16
        if over: pswd += '\0' * (16-over)   # pad: len must be multiple of 16
        aesobj = AES.new(mkey, AES.MODE_ECB)
    ...
    def do_encode(pswd):
        robj = rotor.newrotor(mkey)         # use enigma encryption
        return robj.encrypt(pswd)
    
    def do_decode(pswd):
        robj = rotor.newrotor(mkey)
    ...
    def do_encode(pswd):
        ...
        res = ''
        for char in pswd:
            res += chr(ord(char) + adder)       # inc eac ASCII code
        return str(res)
