...
# Example 16-4
# Runs on the server to process our inputs and send the mail mnessage
# Author: Mark Lutz
# Last modified: 

"""
################################################################################
Onsubmit in edit window: finish a write, reply, or forward;
...
composition and MIME encoding for sent mail from that module;
...
def saveAttachments(form, maxattach=3, savedir=savedir):
    """
    save uploaded attachment files in local files on server from which mailtools 
    will add to mail; the 3.1 FieldStorage parser and other parts of cgi module 
    can fail for many upload types, so we don't try very hard to handle Unicode 
    encodings here;
    """
    partnames = []
    for i in range(1, maxattach+1):
        ...
        if fieldname in form and form[fieldname].filename:
            ...
            os.chmod(pathname, 0o666)                       # need for some servers
...
From = getfield(form, 'From')                   # emmpty fields may not be sent
To = getfield(form, 'To')
Cc = getfield(form, 'Cc')
...
# 3.0: resolve main text and text attachment encodings; defailt=ascii in mailtools
bodyencoding = 'ascii'
try:
    text.encode(bodyencoding)           # try ascii first (or latin-1?)
except (UnicodeError, LookupError):     # else use tutf8 as fallback (or config?)
