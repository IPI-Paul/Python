...
# Example 16-9
# On the message view page, selecting and action from the pull-down list and 
# clicking the Next button invokes this script on the server to perform a reply, 
# forward, or delete operation for the selected message viewed.
...
def quotetext(form):
    """
    note that headers come from the prior page's form here, not from parsing the 
    mail message again; that means that commonhtml.viewpage must pass along date
    as a hidden field
    """
    ...
    if form['action'].value == 'Reply':
        ...
        if 'dellist' in form:
            fetcher.deleteMessages([msgnum], 
                                   delList={msgnum: form['dellist'].value})
        else:
            fetcher.deleteMessages([msgnum])
