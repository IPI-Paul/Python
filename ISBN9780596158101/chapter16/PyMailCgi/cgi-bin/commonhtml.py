...
def pageheader(app='PyMailCGI', color='#FFFFFF', kind='main', info=''):
    ...
    print(tab, '<body bgcolor="%s">\n'% color, tab*2, '<h1>\n', tab*3, 
          '%s %s\n' % (app, (info or kind)), tab*2, 
          '</h1>\n', tab*2, '<hr />')

def pagefooter(root='pymailcgi.html'):
    print(tab*2, '</p>\n', tab*2, '<hr />\n', tab*2, 
          '<a href="http://www.python.org">')
    print(tab*3, '<img src="../PythonPoweredSmall.gif" ', end='')
    print('align="left" alt="[Python Logo]" border="0" hspace="15">\n', tab*2, 
          '</a>')
    print(tab*2, '<a href="../%s">Back to root page</a>' % root)
    print(tab, '</body>\n','</html>')
...
def messagearea(headers, text, extra=''):               # extra for readonly
    ...
    print(tab*3, '<p>\n', tab*4, '<table border cellpadding="3">')
    ...
    if extra == 'readonly':
        ...
        print(tab*6, '<td>\n', tab*7, '<textarea name="text" cols="80" rows="10" %s>'
              % extra, '%s\n' % (cgi.escape(text) or '?'), tab*7, '</textarea>\n',
              tab*6, '</td>')
...
def viewpage(msgnum, headers, text, form, parts=[]):
    ...
    print(tab, '<script>\n', tab*3,'function actn() {\n', tab*4,
          'if (document.getElementById("action").value == "Reply" || ',
          'document.getElementById("action").value == "Forward") {\n', tab*5,
          'document.getElementById("vfrm").target = "_blank";\n', tab*4, '}\n',
          tab*4, 'else {\n', tab*5, 
          'document.getElementById("vfrm").target = "_self";\n', tab*4, '}\n',
    ...
    print(tab*7, '<option>\n', tab*8, 'Reply\n', tab*7, '</option>\n', tab*7,
          ...
          '<option>\n', tab*8, 'Delete\n', tab*7, '</option>\n', tab*6, 
          '</select>')
    print(tab*6, '<input type="button" onclick="actn()" value="Next" />\n', tab*5, '</td>\n', 
          tab*4, '</tr>')
...
def confirmationpage(kind):
    ...
    print(tab*2, '<p>\n', tab*3, 'Press the link below to return to the main page.',
          tab*2, '</p>')
    pagefooter()
